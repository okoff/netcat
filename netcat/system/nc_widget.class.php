...
class nc_Widget extends nc_System {
    ...
    public function load($load = 'widgetclasses') {
        switch ($load) {
            case 'widgetclasses':
                $widgetclasses = $this->core->db->get_results("SELECT *, count(`Field_ID`) AS Fields FROM `Widget_Class` LEFT JOIN `Field` USING (`Widget_Class_ID`)GROUP BY `Widget_Class_ID`");
