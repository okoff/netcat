...
class nc_Input extends nc_System {
    ...
    public function prepare_extract() {
        ...
        if ( isset($nc_core->security) && $nc_core->SECURITY_XSS_CLEAN ) {
			...
			$_COOKIE = array_map_recursive( array('nc_Security', 'xss_clean'), $_COOKIE );
			$_GET = array_map_recursive( array('nc_Security', 'xss_clean'), $_GET );
			$_ENV = array_map_recursive( array('nc_Security', 'xss_clean'), $_ENV );
			$_SERVER = array_map_recursive( array('nc_Security', 'xss_clean'), $_SERVER );
		}
