...
class nc_Trash extends nc_Essence {
    ...
    `sub`.`EnglishName`, `sub`.`Checked`,
    `cc`.`Sub_Class_Name`, `cc`.`EnglishName` AS `ccEnglishName`, `cc`.`Checked` AS `ccChecked`
    ...
    WHERE `Message_ID` IN ( ".join(',', $messages)." ) ", ARRAY_A);
        ...
        // берем последний файл из корзины данного компонента, смотрим размер и соответсвие полей
        $new_file = 1;
        ...
        foreach ($sub_classes as $cc_id => $v) {
            if (!$new_file && ($t = $xpath->query("/netcatml/sub_classes/sub_class[@sub_class_id='".$sub_id."']")) && $t->length) {
                continue;
            }
    ...
    SELECT `Trash_ID`, `Message_ID`, `Class_ID`, `Subdivision_ID`, `Sub_Class_ID`,
    ...
    WHERE `Trash_ID` IN (".join(', ', $trash_ids).")", ARRAY_A);
        ...
        foreach ($trashed_objects as $v) {
            ...
            $filepath = $v['Class_ID'].'/'.$v['XML_Filename'];
        ...
        $xml_exists = $db->get_col("SELECT DISTINCT (CONCAT(Class_ID, '/', XML_Filename)) FROM Trash_Data");
        if (!$xml_exists) $xml_exists = array();

        $file_to_del = array_diff($xml_filepaths, $xml_exists);
