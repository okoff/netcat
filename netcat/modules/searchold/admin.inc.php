...
function last_indexed(){
    ...
    $last=$db->get_var("SELECT MAX(Created) from Message".$MODULE_VARS['searchold']['INDEX_TABLE']);
    return $last;
}
...
function getBytes(){
    global $db,$MODULE_VARS;
    $last=$db->get_var("SELECT SUM(Size) from Message".$MODULE_VARS['searchold']['INDEX_TABLE']);
    return $last;
}
...
function getTotal(){
    global $db,$MODULE_VARS;
    $last=$db->get_var("SELECT count(*) from Message".$MODULE_VARS['searchold']['INDEX_TABLE']);
    return $last;
}
...
function ShowBrokenLinks() {
	...
	$select="SELECT URL, Source, Message_ID FROM Message".$MODULE_VARS['searchold']['INDEX_TABLE']." WHERE Checked=0 ORDER BY URL";
	...
	if ($Result = $db->get_results($select,ARRAY_N)) {
		...
		echo "<td bgcolor=eeeeee><font size=-2><b>".NETCAT_MODULE_SERCH_LINK."</td>\r\n";
		echo "<td bgcolor=eeeeee><font size=-2><b>".NETCAT_MODULE_SERCH_REFERRERS."</td>\r\n";
		echo "</tr>\r\n";
...
//    $contents = join ("", $contents);
    ...
    if ($contents) {
        ...
        nc_preg_match_all("/<a\s+[^>]*href\s*=\s*['\"]?([^ \"'>\t#]+)/ims", $contents, $document_url);
        ...
        if ($searchable_content)
        {
           ...
           $searchable_content = nc_preg_replace($search_re, $replace_re, $searchable_content);
            ...
            if (!$no_index) {
        	    ...
        	    // links with ./ and ../
                    ...
                    while (strpos($real_url, "/../")) {
                       $real_url = nc_preg_replace("!/[^/]+/\.\./!", "/", $real_url);
                    }
