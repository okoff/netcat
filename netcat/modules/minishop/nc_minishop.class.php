<?php
/* $Id: nc_minishop.class.php 5587 2011-11-01 09:09:16Z gaika $ */
...
class nc_minishop {
 ...
 // требовать
    ...
    public function show_put_button($name, $price, $uri = '', $mass_id = 0, $incart = -1) {
        ...
        if ($uri === 'this') $uri = $_SERVER['REQUEST_URI'];
    ...
    public function suitable_discount($sum = 0) {
        if (!$this->settings['discount_enabled']) return 0;
        if (!$sum) $sum = $this->cart_sum();

        $d = array();
        $d_str = $this->settings['discounts'];
        if ($d_str) $d = unserialize($d_str);
        foreach ($d as $v) {
            if ($v['from'] <= $sum && $sum <= $v['to']) return $v['value'];
        }
        return 0;
    }
    ...
    public function update_urls() {
        ...
        $order_info = $this->db->get_row("SELECT cc.`Sub_Class_ID`, sub.`Hidden_URL`, cc.`EnglishName` FROM `Sub_Class` as cc, `Subdivision` as sub WHERE cc.Class_ID = '".intval($order_class_id)."' AND cc.`Subdivision_ID` = sub.`Subdivision_ID` ORDER BY cc.Class_Template_ID LIMIT 1", ARRAY_A);
        if ($order_info) {
            $nc_core->set_settings('order_url', $order_info['Hidden_URL'], 'minishop');
            $nc_core->set_settings('addorder_url', $order_info['Hidden_URL'].'add_'.$order_info['EnglishName'].'.html', 'minishop');
            $nc_core->set_settings('order_cc', $order_info['Sub_Class_ID'], 'minishop');
        }

        // обновление ссылок с корзиной
        $cart_info = $this->db->get_row("SELECT cc.`Sub_Class_ID`, sub.`Hidden_URL`, cc.`EnglishName` FROM `Sub_Class` as cc, `Subdivision` as sub WHERE cc.Class_ID = '".intval($cart_class_id)."' AND cc.`Subdivision_ID` = sub.`Subdivision_ID` ORDER BY cc.Class_Template_ID LIMIT 1", ARRAY_A);
        if ($cart_info) {
            $nc_core->set_settings('cart_url', $cart_info['Hidden_URL'], 'minishop');
    ...
    public function update_sub($cat_id, $sub_id) {
        if (!is_array($sub_id)) $sub_id = array($sub_id);
