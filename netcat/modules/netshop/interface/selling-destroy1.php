...
while ($row = mysql_fetch_array($result)) {	
	...
	while ($row1 = mysql_fetch_array($res1)) {	
		// удаляем из списаний
		$sql2="DELETE FROM Waybills_selling WHERE waybill_id={$row['id']} AND item_id={$row1['item_id']}";
		// удаляем из позиции накладной списание
		$sql3="UPDATE Waybills_goods SET sales=0,paid=0 WHERE id={$row1['id']}";
		//echo $sql2."<br>".$sql3."<br><br>";
		if (!mysql_query($sql2)) {	
		...
		$sql4="UPDATE Netshop_OrderGoods SET sales=0,closed=0 WHERE item_id={$row1['item_id']}";
		if (!mysql_query($sql4)) {	
			die($sql4."<br>Ошибка: ".mysql_error());
		}
		$sql5="UPDATE Retails_goods SET sales=0,closed=0 WHERE item_id={$row1['item_id']}";
		if (!mysql_query($sql5)) {	
