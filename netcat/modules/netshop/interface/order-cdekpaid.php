...
function getOrderListForm($orders=array()) {
	...
	<form method='post' name='frm1' id='frm1' action='/netcat/modules/netshop/interface/order-cdekpaid.php'  onsubmit='return clickEdit();'>
...
function getPostFileList() {
	...
	if ($result=mysql_query($sql)) {
		while($row = mysql_fetch_array($result)) {
			...
			<td><a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=save&oid={$row['id']}'><img src='/images/buttons/download.png' border='0' alt='Скачать файл' title='Скачать файл'></a></td>
			<td style='width:400px;'>{$reqstr}</td>
			<td>{$row['reqcount']}</td>
			<td>{$row['reqinsurance']}</td>
			<td>{$row['reqnp']}</td>
			<td>{$row['reqdeliverycost']}</td>
			<td>".(($row['answered']!="0000-00-00 00:00:00") ? date("d.m.Y", strtotime($row['answered'])) : "<a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=upload&id={$row['id']}'><img src='/images/buttons/upload.png' border='0' alt='Загрузить файл' title='Загрузить файл' style='display:block;margin:0 auto;'></a>")."
			<a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=upload&id={$row['id']}'><img src='/images/buttons/upload.png' border='0' alt='Загрузить файл' title='Загрузить файл' style='display:block;margin:0 auto;'></a>
			</td>
			...
			<td><a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=difference&oid={$row['id']}'>&gt;&gt;&gt;</a></td>
			</tr>";
			$i=$i+1;
		}
	} else {
...
function saveCSVFile($dir, $dirlink, $orders) {
	...
	$html.="<br><br><a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=view'><b>Список файлов для почты</b></a>";
...
function readCSVFile($dir, $dirlink, $id) {
	...
	$html.="<a target='_blank' href='{$dirlink}{$filename}'><b>Скачать файл</b></a>";
	$html.="<br><br><a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=view'><b>Список файлов СДЭК</b></a>";
	return $html;
}
function printUploadFrm($id) {
	$html="<form enctype='multipart/form-data' action='/netcat/modules/netshop/interface/order-cdekpaid.php' method='post' name='frm1' id='frm1'>
...
function getNarvFileList($dir) {
	...
	foreach ($files as $file) {
		$html.="<li><a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=narv&f={$file['name']}'>".$file['name']."</a></li>";
	}
	$html.="</ul>";*/
	$html.="<form enctype='multipart/form-data' action='/netcat/modules/netshop/interface/order-cdekpaid.php' method='post' name='frm1' id='frm1'>
		<input type='hidden' name='action' value='upload'>
	...
	$barcode="";
	$sql="SELECT * FROM Netshop_CDEKPaid ORDER BY id DESC";
	$html.="<table cellpadding='3' cellspacing='0' border='1'>
	<tr><td><b>#</b></td>
	<td><b>дата загрузки</b></td>
	<td><b>название файла</b></td>
	<td><b>итого</b></td>
	<td><b>заказы в отчете</b></td></tr>";
	if ($result=mysql_query($sql)) {
		while($row = mysql_fetch_array($result)) {
			$barcode="";
			...
			$stro="";
			$tmp=explode(";",$row['orders']);
			foreach ($tmp as $t) {
				if ($t!="") {
					$stro.="<a target='_blank' href='/netcat/message.php?catalogue=1&sub=57&cc=53&message=".$t."'>".$t."</a> | ";
				}
			}
			$html.="<tr>
			<td><a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=narv&id={$row['id']}'>{$row['id']}</a></td>
			<td><a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=narv&id={$row['id']}'>".date("d.m.Y", strtotime($row['created']))."</a></td>
			<td>{$row['fname']}</td>
			<td>{$row['itogo']}</td>
			<td>{$stro}</td>
...
function printNarvById($id) {
	$html="";
	$sql="SELECT * FROM Netshop_CDEKPaid WHERE id=".$id;
	if ($result=mysql_query($sql)) {
		$row = mysql_fetch_array($result);
		$html.="<h2>Отчет об оплате #{$row['id']}. {$row['fname']}</h2>";
		$html.=printNarv($row);
		return $html;
	}
...
function printNarv($row) {
	...
	$strs=explode("\n",$row['description']);
	//print_r($strs);
	$html.="<table cellpading='1' cellspacing='0' border='1'>";
	foreach ($strs as $str) {
		//echo $str."<br>";
		$html.="<tr>";
		$tmp=explode(";",$str);
...
function processNarv($fname) {
	$html="";
	$html.="<h3>СДЭК. Обработка отчета об оплате</h3>";
	$text=readTxtFile($_SERVER['DOCUMENT_ROOT'].'/netcat_files/cdekfiles/narv/'.$fname);
	// save data in DB
	$sql="INSERT INTO Netshop_CDEKPaid (fname, description, created) VALUES
		('{$fname}','".convstr($text)."','".date("Y-m-d H:i:s")."')";
	if (mysql_query($sql)) {
		$html.= "Файл отчета об оплате сохранен.<br>";
		
	} else {
		die($sql."<br>Ошибка: " . mysql_error());
	}
	$fileid=0;
	$sql="SELECT * FROM Netshop_CDEKPaid ORDER By id DESC LIMIT 1";
	if ($result=mysql_query($sql)) {
		while($row = mysql_fetch_array($result)) {
			$fileid=$row['id'];
		}
	...
	$strs=explode("\n", $text);
	$numstr=count($strs);
	$itog = $strs[count($strs)-2];
	$tmpitog=explode(";",$itog);
	//print_r($tmpitog);
	//$itogsum=str_replace("Итого к перечислению","",$tmpitog[0]);
	
	//$itogsum=substr($tmpitog[0],20);
	$itogsum=$tmpitog[1];
	$itogsum=trim(str_replace(",",".",$itogsum));
	//echo $itogsum."<br>";
	$html.="Файл отчета обработан<br>";
	$sql="UPDATE Netshop_CDEKPaid SET itogo=".$itogsum." WHERE  id=".$fileid;
	if (mysql_query($sql)) {
		//$html.= "<br>
		//<p><a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=narv&id=".$fileid."'>Просмотр отчета</a></p>";
		
	} else {
		echo ($sql."<br>Ошибка: " . mysql_error());
	}
	...
	for ($i=0;$i<(count($strs)-2);$i++) {
		...
		//echo "<br><br>";
		//if (strlen($tmp[0])>3) {
		$order_id=intval($tmp[1]);
		//echo stripos(convstr($tmp[7]),"вручен");
		//echo "<br><br>";
		if ($order_id>0) {
			//echo $order_id."-".convstr($tmp[7])."<br>";
			if (stripos(convstr($tmp[7]),"вручен")>1) {
				$html.= "<b style='color:#f30000;'> Заказ {$order_id} не вручен!</b><br>";
			} else {
				$sql="SELECT * FROM Message51 WHERE Message_id ={$order_id} LIMIT 1";

				if ($result=mysql_query($sql)) {
					while($row = mysql_fetch_array($result)) {
						//print_r($row);
					
						$paysum=str_replace(",",".",$tmp[8]);
						...
						$sql="UPDATE Message51 SET acceptdate='".date("Y-m-d H:i:s",strtotime($tmp[4]))."', 
									paydate='".date("Y-m-d H:i:s",strtotime($tmp[4]))."',
									paysum=".$paysum.", Status=4, paid=1,
									wroff=1,wroffdate='".date("Y-m-d",strtotime($tmp[4]))."' WHERE Message_ID={$row['Message_ID']}";
						//echo $sql."<br>";
						if (mysql_query($sql)) {
							$ordersstr.=$order_id.";";
	...
	$html.="Файл отчета обработан<br>";
	$sql="UPDATE Netshop_CDEKPaid SET orders='".$ordersstr."' WHERE  id=".$fileid;
	if (mysql_query($sql)) {
		$html.= "<br>
		<p><a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=narv&id=".$fileid."'>Просмотр отчета</a></p>";
...
// ------------------------------------------------------------------------------------------------
	...
	$UploadDir=$_SERVER['DOCUMENT_ROOT'].'/netcat_files/cdekfiles/upload/';
	$UploadDirNarv=$_SERVER['DOCUMENT_ROOT'].'/netcat_files/cdekfiles/narv/';
	//$UploadDir='/netcat_files/postfiles/upload/';
	//$DownloadDir=$_SERVER['DOCUMENT_ROOT'].'/netcat_files/postfiles/';
	$DownloadDir=$_SERVER['DOCUMENT_ROOT'].'/netcat_files/cdekfiles/';
	...
	switch ($incoming['action']) {
		...
		case "save":
			...
			//header("Location:/netcat/modules/netshop/interface/order-cdekpaid.php?action=view");
			break;
		case "upload":
			if ($incoming['request_method']=="get") {
				$html=printUploadFrm($incoming['id']);
				if (file_exists($uploadfile)) {
					$html.="<p>Файл загружен. <a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=process&id={$incoming['id']}'>Начать обработку файла.</a></p>";
				}
			} else {
				if (file_exists($uploadfile)) {
					unlink($uploadfile);
				...
				if ((isset($incoming['id'])) && ($incoming['id']!="narv")) {
					...
					if (move_uploaded_file($_FILES['fname']['tmp_name'], $UploadDirNarv.$_FILES['fname']['name'])) {
						//$html.="<p>Файл корректен и был успешно загружен.</p>";
						$html.="<p>Файл загружен. <a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=narv&f={$_FILES['fname']['name']}'>Начать обработку файла.</a></p>";
				...
				/*	//print_r($_FILES);
					...
					$html="<p>Файл загружен. <a href='/netcat/modules/netshop/interface/order-cdekpaid.php?action=process&id={$incoming['id']}'>Начать обработку файла.</a></p>";
					//header("Location:/netcat/modules/netshop/interface/order-cdekpaid.php?action=process&id={$incoming['id']}");
				}*/
			}
			break;
		case "process":
		...
		default:
			...
			if (isset($incoming['id'])) {
				...
				$html.=getNarvFileList("../../../../netcat_files/cdekfiles/narv/");
			}
			break;
		//default:
