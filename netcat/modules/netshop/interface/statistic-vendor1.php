...
function showStatisticForVendorShort($sid,$min,$max, $incoming=array()) {
	...
	$sql="SELECT Retails_goods.*,Message57_p.Price AS PriceZ,Retails.created AS RetCreated,Message57.ItemID,Message57.Name,Message57.StockUnits,Message57.Price,Message57.Subdivision_ID,Message57.Sub_Class_ID,Message57.complect,Message57.vendor_itemid  FROM Retails_goods 
			INNER JOIN Message57 ON ( Retails_goods.item_id = Message57.Message_ID )
			LEFT JOIN Message57_p ON ( Message57_p.Message_ID = Message57.Message_ID )
			INNER JOIN Retails ON ( Retails.id = Retails_goods.retail_id )
			WHERE Retails.created BETWEEN '".date("Y-m-d 00:00:00", strtotime($min))."' AND '".date("Y-m-d 23:59:59", strtotime($max))."'
	...
	$sql="SELECT Netshop_OrderGoods.*, Message51.created AS OrdCreated,Message57.ItemID AS ART,Message57_p.Price AS PriceZ,Message57.Name,Message57.StockUnits,Message57.Price,Message57.Subdivision_ID,Message57.Sub_Class_ID,Message57.complect,Message57.vendor_itemid 
			FROM Netshop_OrderGoods 
			INNER JOIN Message57 ON ( Netshop_OrderGoods.Item_ID = Message57.Message_ID )
			INNER JOIN Message57_p ON ( Netshop_OrderGoods.Item_ID = Message57_p.Message_ID )
			INNER JOIN Message51 ON ( Message51.Message_ID = Netshop_OrderGoods.Order_ID )
	...
	if ((isset($incoming['sort'])==0) || ((isset($incoming['sort'])!=0)&&($incoming['sort']=="art"))) {
		...
		foreach ($items as $key => $val) {
			//echo "$key = $val\n";
			$i=$i+1;
			$res.="<tr><td>{$i}</td><td>{$val}</td><td>{$art[$key]}</td><td>{$num[$key]}</td><td>{$data[$key]}</td><td>{$rest[$key]}</td><td>{$pricez[$key]}</td><td>{$price[$key]}</td></tr>";
		}
	}
	if ((isset($incoming['sort'])!=0)&&($incoming['sort']=="sold")) {
		...
		foreach ($num as $key => $val) {
			//echo "$key = $val\n";
			$i=$i+1;
			$res.="<tr><td>".$i."</td><td>".$items[$key]."</td><td>".$art[$key]."</td><td>".$val."</td><td>".$data[$key]."</td><td>".$rest[$key]."</td><td>".$pricez[$key]."</td><td>".$price[$key]."</td></tr>";
		}
	}
	if ((isset($incoming['sort'])!=0)&&($incoming['sort']=="stock")) {
		...
		foreach ($rest as $key => $val) {
			//echo "$key = $val\n";
			$i=$i+1;
			$res.="<tr><td>".$i."</td><td>".$items[$key]."</td><td>".$art[$key]."</td><td>".$num[$key]."</td><td>".$data[$key]."</td><td>".$val."</td><td>".$pricez[$key]."</td><td>".$price[$key]."</td></tr>";
		}
	}
	$res.="</table>";
	$all=mysql_num_rows($r)+mysql_num_rows($r1);
...
<body>
	<?php
		echo printMenu();
	?>
	<h1>Статистика по поставщику. Проданные товары</h1>
	<form name="frm1" id="frm1" action="/netcat/modules/netshop/interface/statistic-vendor1.php" method="post">
	...
	<table cellpadding="2" cellspacing="0" border="1">
		...
		<td>
		<input type="checkbox" value="1" id="short" name="short" <?php echo ((isset($incoming['short'])) ? "checked" : ""); ?>>свернуть
		</td></tr>
		<tr>
			<td style="text-align:right;">Поставщик:</td>
			<td><?php echo selectSupplier($incoming); ?></td>
			<td><input type="checkbox" value="1" id="clnt" name="clnt" <?php echo ((isset($incoming['clnt'])) ? "checked" : ""); ?>>клиенты</td>
		</tr>
		<tr>
