...
function getCardsList($incoming) {
	...
	$cnum=0;
	$sql="SELECT *
		FROM Action_RVS {$strwhere}
		ORDER BY cardnum ASC";
	//echo "<br>".$sql;
	$html="<p>[<b><a href='/netcat/modules/netshop/interface/action-rvs.php?id=".(getLastInsertID("Action_RVS","id")+1)."&action=edit'>Добавить</a></b>]</p>
	...
	if ($result=mysql_query($sql)) {
		while ($row=mysql_fetch_array($result)) {
			$html.="<tr>
			<td><a href='/netcat/modules/netshop/interface/action-rvs.php?id={$row['id']}&action=edit'>{$row['cardnum']}</a></td>
			<td>{$row['uname']}</td>
			<td>{$row['uphone']}</td>
			<td>{$row['email']}</td>
...
function editCard($id=0) {
	...
	$html="";
	//print_r($_SESSION);
	$sql="SELECT * FROM Action_RVS where id=".$id;
	if ($result=mysql_query($sql)) {
	...
	<form action='/netcat/modules/netshop/interface/action-rvs.php' method='post'>
	...
	switch ($incoming['action']) {
		case "save":
			...
			if (is_numeric($incoming['id'])) {
				...
				//echo $created."<br>";
				$sv=true;
				$sql="SELECT cardnum FROM Action_RVS WHERE NOT id=".intval($incoming['id']);
				if ($result=mysql_query($sql)) {
					while ($row=mysql_fetch_array($result)) {
						//echo $row['id']."<br>";
							if ($row['cardnum']==$incoming['cardnum']) {
								...
								$html.="<p>Карта с таким номером существует!</p><p><a href='/netcat/modules/netshop/interface/action-rvs.php?id={$incoming['id']}&action=edit'>Вернуться назад.</a></p>";
								$sv=false;
								break;
							}
				...
				if ($sv) {
					$all=getLastInsertID("Action_RVS","id");
					if ($incoming['id']<=$all) {
						$sql="UPDATE Action_RVS SET 
						...
						$sql="INSERT INTO Action_RVS (cardnum,uname,uphone,email,
					...
					$html.="<p>Изменения сохранены.</p><p><a href='/netcat/modules/netshop/interface/action-rvs.php'>Вернуться в список карт.</a></p>";
				}
			}
			$show=0;
			break;
		case "del":
			if (isset($incoming["id"])) {
				$sql="";
				if ($incoming["img"]==1) {
					$sql="UPDATE Action_RVS SET certificate='' WHERE model_ID=".intval($incoming['id']);
				}
				if ($incoming["img"]==2) {
					$sql="UPDATE Action_RVS SET certificate1='' WHERE model_ID=".intval($incoming['id']);
				}
				if ($incoming["img"]==3) {
					$sql="UPDATE Action_RVS SET certificate2='' WHERE model_ID=".intval($incoming['id']);
				}
				if ($incoming["img"]==4) {
					$sql="UPDATE Action_RVS SET certificate3='' WHERE model_ID=".intval($incoming['id']);
				}
				if ($incoming["img"]==5) {
					$sql="UPDATE Action_RVS SET certificate4='' WHERE model_ID=".intval($incoming['id']);
				}
				/*if ($sql) {
					if (!mysql_query($sql)) {
						die(mysql_error());
				...
				$html.="<p><a href='/netcat/modules/netshop/interface/action-rvs.php'>Вернуться в список моделей.</a></p>";
			}
			$show=0;
			break;
		case "edit":
