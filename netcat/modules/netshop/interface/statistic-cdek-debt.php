...
function showListFiles() {
	...
	$sql="SELECT * FROM Netshop_CDEKFiles ORDER BY id ASC";
	$result=mysql_query($sql);
...
function getOrderCostNPCdek($oid) {
	$cost=0;
	$sql="SELECT DeliveryCost FROM Message51 WHERE Message_ID=".$oid;
	$result=mysql_query($sql);
	while ($row = mysql_fetch_array($result)) {
		$cost=$row['DeliveryCost'];
	}
	$cost=$cost+getOrderCost($oid);
	return $cost;
}
function showOrdersOnPostDebt($incoming) {
	$res="";
	$sql="SELECT * FROM Netshop_CDEKFiles WHERE created BETWEEN '".date("Y-m-d", strtotime($incoming['min']))."' AND '".date("Y-m-d", strtotime($incoming['max']))."' 
		ORDER BY id DESC";
	//echo $sql;
	$res.="<table cellpadding='2' cellspacing='0' border='1'>";
	$res.="<tr style='font-weight:bold;'><td>#</td><td>#</td><td>Дата отправки</td>
		<td>Кол. заказов</td>
		<td>Общая сумма по заказам</td>
		<td>Кол. оплаченных заказов</td>
	...
	while ($row = mysql_fetch_array($result)) {
		...
		$ans=explode(";", $row['ppfile']);
		foreach($ans as $ord) { 
			if ($ord) {
				$itl1=$itl1+getCountCart($ord);
				$summord=getOrderCostNPCdek($ord);
				$summ=$summ+$summord;
				if (ifOrderPaid($ord)) {
		...
		<td style='text-align:right;'>".(count($ans)-1)."</td>
	...
	$res.="<tr style='font-weight:bold;'><td colspan='3'>Итого:</td>
...
<head>
	...
	<title>Статистика по отправлениям СДЭК</title>
	<style>
	body, td {
		font-size:10pt;
		font-family:Tahoma;
...
<body>
	<?php
		echo printMenu();
	?>
	<h1>Статистика по отправлениям СДЭК (долги)</h1>
	<form name="frm1" id="frm1" action="/netcat/modules/netshop/interface/statistic-cdek-debt.php" method="post">
	<input type="hidden" name="action" id="action" value="filter">
	<table border="0" cellpadding="0" cellspacing="5">
		<tr>
			<td>с</td>
			<td><input name="min" value="<?php echo isset($incoming['min']) ? date("d.m.Y", strtotime($incoming['min'])) : "01.01.2017" ?>" class="datepickerTimeField"></td>
			<td>по</td>
