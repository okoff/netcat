...
<Ads formatVersion=\"3\" target=\"Avito.ru\">";
        ...
        foreach ($structure as $row) {
			...
			if (($row["Subdivision_ID"]==2561)||
					($row["Subdivision_ID"]==2562)||($row["Subdivision_ID"]==2563)||
				($row["Subdivision_ID"]==2710)||
					($row["Subdivision_ID"]==2711)||($row["Subdivision_ID"]==2712)||($row["Subdivision_ID"]==2713)||	
				($row["Subdivision_ID"]==2751)||
					($row["Subdivision_ID"]==2747)||($row["Subdivision_ID"]==2748)||($row["Subdivision_ID"]==2749)||($row["Subdivision_ID"]==2750)
					) {
 ...
 //               "VendorCode" => "vendorCode",
    ...
    <Ad>
		...
		<Title>".$name."</Title>
        <Description>".xmlspecialchars(strip_tags($this->convstrw_li($row['Name']))).". ".xmlspecialchars(strip_tags($this->convstr_li($row['Description'])))."Арт. ".xmlspecialchars(strip_tags($this->convstrw_li($row['ItemID'])))." </Description>
		...
		Title=".$namev."<br>
        Description=".xmlspecialchars(strip_tags($row['Name'])).". ".xmlspecialchars(strip_tags($row['Description']))." ".$this->convstr_li("Арт").xmlspecialchars(strip_tags($row['ItemID']))."<br>
		Price=".$row['Price']."<br>
		&lt;Image url=\""."http://".$HTTP_HOST.$SUB_FOLDER.nc_file_path($class_id, $row["Message_ID"], "Image", "h_")."\" /&gt;
    <br><br>";
					$ii=$ii+1;
