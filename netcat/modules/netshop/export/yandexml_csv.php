...
class Netshop_ExportYML extends Netshop {
    ...
    public function ExportYML($section=0) {
        ...
        /*$ret = (strftime("%Y-%m-%d %H:%M"))."<br>".
				xmlspecialchars($shopName)."<br>".
                xmlspecialchars($this->CompanyName)."<br>".
                $HTTP_HOST.$SUB_FOLDER."<br>";*/
      /*            <currencies>
...
*/
        ...
        foreach ($structure as $row) {
			...
			if (($row["Subdivision_ID"]!=153)&&($row["Subdivision_ID"]!=58)
				...
				//$ret .= $row['Subdivision_ID'].";";

				if (array_key_exists($row['Parent_Sub_ID'], $structure)) {
					//$ret .= $row['Parent_Sub_ID'].";";
				}

				//$ret .= xmlspecialchars($row["Subdivision_Name"])."<br>";
        ...
        foreach ($goods_class_ids as $class_id) {
            ...
            foreach ((array) $rows as $row) {
				...
				if ((mb_stripos($row['Name'],$this->convstr_li("финск")) === false)&&
					...
					$ret .= sprintf("%d%05d", $class_id, $row["Message_ID"]).";";
					$vendormodel = 0;
					...
					foreach ($output as $idx => $tag) {
						if ($row[$idx]) {
							...
							if ($tag == 'name' && $row['GroupName'] != '') {
								$ret .= xmlspecialchars(strip_tags($row['GroupName']))." - ".xmlspecialchars(strip_tags($value)).";";
							} else {
								if ($vendormodel && $tag == 'name') $tag = 'model';
								$ret .= xmlspecialchars(strip_tags($value)).";";
							}
						}
					}
					$ret .= "<br>";
