...
class nc_comments {
    ...
    public function wall($message_id, $template = 0, $from = 0, $quantity = 0, $reset = 0, $ignore = 0, $show_all = 0) {
        ...
        if ($maxRows && $totRows > $maxRows) {
            ...
            if ($this->settings['ShowAll'] && !$ignore) {
                ...
                $replacing = array("nc_commentsObj" . $this->_message_cc . "_" . $message_id . ".showAll(); return false;");
    ...
    public function listing($message_id) {
        ...
        $replacing = array("nc_commentsObj" . $this->_message_cc . "_" . $message_id . ".navComments(" . $curPos . "); return false;", "nc_commentsObj" . $this->_message_cc . "_" . $message_id . ".navComments(" . $last_pos . "); return false;");
    ...
    public function getComment($message_id, $data, $template_id = 0, $recurse = true, $eval = true) {
        ...
        // demountable values in comment HTML text
        $replaceKeys = array("%ID", "%COMMENT", "%VALUE", "%IS_NEW", "%USER_AVATAR", "%USER_ID", "%USER_NAME", "%GUEST_NAME", "%GUEST_EMAIL", "%DATE", "%UPDATED", "%REPLY_LINK", "%EDIT_LINK", "%DELETE_LINK", "%REPLY_BLOCK", "%LEVEL");
        // comment reply link
        $reply_link = "";
        if ($this->_accessibilityAdd) {
            // demountable values in comment HTML text
    ...
    public function addComment($message, $parent_message, $comment, $user = 0, $nc_comments_guest_name = '', $nc_comments_guest_email = '') {
        ...
        $comment = nc_quote_convert(strip_tags($comment));
        ...
        if (!$this->core->NC_UNICODE) {
            ...
            $comment = $this->core->utf8->utf2win($comment);
        }
        if ($parent_message) {
    ...
    public function editCommentForm($id) {
        ...
        $result = "<form id='adminForm' action ='admin.php' method = 'post'>";
        // author info
        $result .= "<fieldset>\n" .
                "<legend>\n" .
        ...
        if ($comment_data['User_ID'])
            $result .= NETCAT_MODULE_COMMENTS_ADMIN_EDIT_AUTHOR_USER . " <a href='/netcat/admin/user/index.php?phase=4&UserID=" . $comment_data['User_ID'] . "'>" . $comment_data['Login'] . "</a>";
...
function delete_allComments() {
    ...
    // delete comment from base
    $db->query("DELETE FROM `Comments_Text`");
    // zero count
