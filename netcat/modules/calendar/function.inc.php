...
/* $Id: function.inc.php 6485 2012-03-23 06:10:47Z alive $ */

/**
 * Вывод настроек календаря в шапку макета
 * @param int идентификатор темы календаря
...
function nc_set_calendar($id = 0) {
    ...
    if ($css) $result.= "<style type='text/css'>\n".$css."</style>\n";
 ...
 * @param int права на чтение, для этого сайта
 * @param int идентификатор раздела
 * @param int права на чтение, для этого раздела
 * @param int идентификатор компонента в разделе
 * @param int права на чтение, для этого компонента в разделе
 * @return bool
 */
function nc_calendar_read_permission($catalogue, $sub, $cc) {
    ...
    if (!$AUTH_USER_ID || !class_exists("Permission") || !is_object($perm))
            return false;
    if (!$perm->isSubClass($cc, MASK_READ)) return false;
 ...
 * @param int идентификатор темы календаря
 * @param int идентификатор компонента в разделе
 * @param string дата для отображения
 * @param bool заполнить независимо от маски дней
 * @return string html-код календаря
 */
function nc_show_calendar($theme = 0, $cc = 0, $setDate = 0, $DateField = "Date", $filled = false, $queryDate = false, $popup = false, $calendar_fields = array()) {
    global $db, $DOCUMENT_ROOT, $SUB_FOLDER, $HTTP_ROOT_PATH, $action, $nc_core;

    # поле для выборки
    if ($DateField) $DateField = $db->escape($DateField);
    # переменные
    $antiJumper = true;
    $theme = intval($theme);
    $curr_year = date("Y");
    ...
    if (!$popup) {
        ...
        foreach ($cc_ids as $k => $v) {
            ...
            if (!$first_hidden_url)
                    $first_hidden_url = $cc_data[$v]['Hidden_URL'];
            ...
            switch ($cc_data[$v]['Read_Access_ID']) {
                case 1:
                    ...
                    break;
                case 2:
                    if ($AUTH_USER_ID) $read_access = true;
                    break;
                case 3:
                    $read_access = nc_calendar_read_permission($cc_data[$v]['Catalogue_ID'], $cc_data[$v]['Subdivision_ID'], $v);
                    break;
            }
            if (!$read_access) unset($cc_ids[$k]);
        }

        if (empty($cc_ids)) return false;
    }


    // date from adress line
    if ($queryDate || preg_match("/\/(\d{4})\/(?:(\d{2})\/)?(?:(\d{2})\/)?/s", $_SERVER['REQUEST_URI'], $matches)) {
        if (!$queryDate) {
            ...
            if (!$query_date_month) $query_date_month = 0;
            if (!$query_date_day) $query_date_day = 0;
        }
    }
    else {
        $queryDate = false;
    ...
    # 0 - воскресенье
    if ($weekDayBehind == 0) $weekDayBehind = 7;

    # вставляем пустые дни в начало
    for ($i = 1; $i < $weekDayBehind; $i++) {
        # суббота
        if (($i % 7) == 6) $calendarBody.= $settings['nodaySETTemplate'];
        # воскресенье
        elseif (!($i % 7)) $calendarBody.= $settings['nodaySUNTemplate'];
        # дни с понедельника по пятницу
        else $calendarBody.= $settings['nodayTemplate'];
    }
    if ($weekDayBehind != 1) $lines = $lines + 1;
    ...
    if (!$filled) {
        ...
        foreach ($cc_ids as $cc_id) {
            $query[] = "SELECT DAYOFMONTH(`".$cc_date_field[$cc_id]."`) as `d`, `Sub_Class_ID` as `cc`
                ...
                WHERE `Sub_Class_ID` = '".intval($cc_id)."'
                AND MONTH(`".$cc_date_field[$cc_id]."`) = '".$month."'
        ...
        if ($daysMask[$i - 1]) {
            ...
            if ($popup) {
                ...
                if ($nc_core->admin_mode) {
                    $Hidden_URL = $nc_core->SUB_FOLDER.$nc_core->HTTP_ROOT_PATH.'?sub='.$cc_data[$daysMask[$i - 1]]['Subdivision_ID'].'&amp;cc='.$daysMask[$i - 1];
                    $dayLinkBegin = "<a href='".$Hidden_URL.'&amp;date='.$year."-".sprintf("%02d", $month)."-".sprintf("%02d", $i)."'>";
                } else {
                    $Hidden_URL = $cc_data[$daysMask[$i - 1]]['Hidden_URL'];
                    $dayLinkBegin = "<a href='".$SUB_FOLDER.$Hidden_URL.$year."/".sprintf("%02d", $month)."/".sprintf("%02d", $i)."/'>";
                }
            }
        } else {
    ...
    # 0 - воскресенье
    if ($weekDayAfter == 0) $weekDayAfter = 7;
    ...
    for ($i = $weekDayAfter; $i < $daysEnd; $i++) {
        ...
        if (($i + 1) % 7 == 1 && $daysEnd > 7)
                $calendarBody.= $settings['dayTemplateEnd'].$settings['dayTemplateBegin'];
        # суббота
        if ((($i + 1) % 7) == 6) $calendarBody.= $settings['nodaySETTemplate'];
        # воскресенье
        else if (!(($i + 1) % 7)) $calendarBody.= $settings['nodaySUNTemplate'];
        # дни с понедельника по пятницу
        else $calendarBody.= $settings['nodayTemplate'];
    }
    ...
    if (is_array($MonthArray)) $MonthArray = array_pad($MonthArray, -13, "");
    ...
    if ($popup) {
        ...
        $month_select = "<select onchange='nc_calendar_generate(".$day.", this.value, ".$year."); return false;'>";
    }
    ...
    if ($popup) {
        ...
        $year_select = "<select onchange='nc_calendar_generate(".$day.", ".$month.", this.value); return false;'>";
    }
    ...
    if ($popup) {
        ...
        $CalendarHeader = str_replace("%IMG_PREV_MONTH", ($IMG_PREV_MONTH ? "<img src='".$imagesPath.$settings['PrevImage']."' onclick='nc_calendar_generate(".($day ? $day : 1).", ".($month == 1 ? 12 : $month - 1).", ".($month == 1 ? $year - 1 : $year)."); return false;' alt='".$MonthArray[$month == 1 ? 12 : $month - 1]."' title='".$MonthArray[$month == 1 ? 12 : $month - 1]."' />" : "&lt;"), $CalendarHeader);
        $CalendarHeader = str_replace("%IMG_NEXT_MONTH", ($IMG_NEXT_MONTH ? "<img src='".$imagesPath.$settings['NextImage']."' onclick='nc_calendar_generate(".($day ? $day : 1).", ".($month == 12 ? 1 : $month + 1).", ".($month == 12 ? $year + 1 : $year)."); return false;' alt='".$MonthArray[$month == 12 ? 1 : $month + 1]."' title='".$MonthArray[$month == 12 ? 1 : $month + 1]."' />" : "&gt;"), $CalendarHeader);
    }
    $DateFieldExist = true;
    $CalendarHeader = str_replace("%MONTH_LINK", ($DateFieldExist && !$popup ? "<a href='".$nc_core->SUB_FOLDER.$first_hidden_url.$year."/".sprintf("%02d", $month)."/'>" : "").$MonthArray[(int) $month].($DateFieldExist ? "</a>" : ""), $CalendarHeader);
    $CalendarHeader = str_replace("%YEAR_LINK", ($DateFieldExist && !$popup ? "<a href='".$nc_core->SUB_FOLDER.$first_hidden_url.$year."/'>" : "").sprintf("%04d", $year).($DateFieldExist ? "</a>" : ""), $CalendarHeader);
...
function nc_calendar_possibility_check($classID, $DateField = "Date") {
    ...
    if (!$classID || !$DateField) return false;
