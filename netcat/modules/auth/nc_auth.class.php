<?php
/* $Id: nc_auth.class.php 5694 2011-11-16 16:02:57Z gaika $ */
...
class nc_auth {
    ...
    public function get_value($key = '') {
        if (empty($this->settings)) {
            $res = $this->db->get_results("SELECT SettingsKey, Value FROM Auth_Settings", ARRAY_A);
            foreach ($res as $val)
                $this->settings[$val['SettingsKey']] = $val['Value'];
        }
    ...
    public function set_value($key, $value) {
        ...
        $this->db->query("SELECT `SettingsKey` FROM `Auth_Settings` WHERE `SettingsKey` = '".$key."' ");
        if ($this->db->num_rows) {
            $this->db->query("UPDATE `Auth_Settings` SET `Value` = '".$value."' WHERE `SettingsKey` = '".$key."' ");
        } else {
            $this->db->query("INSERT INTO `Auth_Settings`(`SettingsKey`,`Value`)
                  VALUES('".$key."','".$value."') ");
        }
...
NETCAT_ADD_FORM;
	...
	// форма регистрации на сайте russian-knife.ru
...
NETCAT_ADD_FORM;
    ...
    public function add_form_fs() {
        ...
        $fields = $st = $st->get_fields();
        $res = <<<NETCAT_ADD_FORM
<?= \$nc_core->get_settings('deny_reg', 'auth') ? NETCAT_MODULE_AUTH_SELFREG_DISABLED : (\$warnText ? "<div class='warnText'>\$warnText</div>" : NULL ) ?>
<form name='adminForm' id='adminForm' enctype='multipart/form-data' method='post' action='<?= \$nc_core->SUB_FOLDER ?>/netcat/add.php'>
<div id='nc_moderate_form'>
  <div class='nc_clear'></div>
  <input name='admin_mode' type='hidden' value='<?= \$admin_mode ?>' />
...
NETCAT_ADD_FORM;
    ...
    public function auth_form_for_authorized($params = array(), $template = array()) {
        ...
        $login = $current_user[$AUTHORIZE_BY] ? $current_user[$AUTHORIZE_BY] : $current_user['ForumName'];
    ...
    public function get_confirm_mail($user_id) {
        global $SUB_FOLDER;
        ...
        $macro = array('SITE_NAME' => $this->core->catalogue->get_current('Catalogue_Name'),
                ...
                'CONFIRM_LINK' => "http://".$_SERVER['HTTP_HOST'].$SUB_FOLDER."/netcat/modules/auth/confirm.php?id=".$userinfo['User_ID']."&code=".$userinfo['RegistrationCode']);
    ...
    public function get_notify_admin_mail($user_id) {
        ...
        $macro = array('SITE_NAME' => $this->core->catalogue->get_current('Catalogue_Name'),
                ...
                'USER_LOGIN' => $userinfo[$this->core->AUTHORIZE_BY],
                'PASSWORD' => $this->core->input->fetch_get_post('Password1'),
                'CONFIRM_LINK' => "http://".$_SERVER['HTTP_HOST']."/netcat/modules/auth/confirm.php?id=".$userinfo['User_ID']."&code=".$userinfo['RegistrationCode']);
    ...
    public function login_form($IsReturn = 0) {
        ...
        global $db, $catalogue, $sub, $cc, $AUTH_USER_ID, $AUTHORIZATION_TYPE, $ADMIN_AUTHTYPE, $REQUESTED_BY;
        global $nc_auth, $nc_auth_fb, $nc_auth_vk, $nc_auth_openid, $nc_auth_twitter;

        if ($AUTH_USER_ID) return;

        if (!$REQUESTED_FROM) $REQUESTED_FROM = $REQUEST_URI;
        if (!isset($REQUESTED_BY) || isset($REQUESTED_BY) && (strtoupper($REQUESTED_BY) != 'GET' && strtoupper($REQUESTED_BY) != 'POST'))
