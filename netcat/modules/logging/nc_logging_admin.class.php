...
class nc_logging_admin {
     ...
     *
     */
    public function show_logging_data($curPos = 0) {
        // system superior object
        $nc_core = nc_Core::get_object();
      ...
      <col width='3%'/><col/><col width='60%'/><col width='8%'/><col width='8%'/>
        <tr>
          <td nowrap class='align-center><font size='-1'>".NETCAT_MODULE_LOGGING_DATA_ID."</font></td>
          <td nowrap><font size='-1'>".NETCAT_MODULE_LOGGING_DATA_EVENT."</font></td>
          <td nowrap><font size='-1'>".NETCAT_MODULE_LOGGING_DATA_INFO."</font></td>
          <td nowrap><font size='-1'>".NETCAT_MODULE_LOGGING_DATA_DATE."</font></td>
          <td nowrap class='last_col align-center'><font size='-1'>".NETCAT_MODULE_LOGGING_DATA_USER."</font></td>
        </tr>";
            ...
            foreach ($data as $row):
                ...
                // row element
                echo "<tr>".
                "<td class='align-center' bgcolor='".$color."'>".$row['ID']."</td>".
                "<td  nowrap>".$nc_core->event->event_name($row['Event'])."</td>".
                "<td >".$row['Info']."</td>".
                "<td nowrap><font size='-2'>".$row['Date']."</font></td>".
                "<td class='last_col align-center'><font size='-2'>".($row['User_ID'] ? "<a href='".$nc_core->ADMIN_PATH."user/index.php?phase=4&UserID=".$row['User_ID']."'>".$row['Login']."</a>" : NETCAT_MODULE_LOGGING_DATA_SYSTEM)."</font></td>".
                "</tr>";
