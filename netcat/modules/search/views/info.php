...
if (!count($rules)) {
    ...
    $pending_tasks = $db->get_var(
                    "SELECT `StartTime` FROM `Search_Schedule` WHERE `StartTime` < $pending_time LIMIT 1");
    if ($pending_tasks) {
...
if ($has_history) {
    ...
    "</div>",
    "<table class='list'><tr>",
    "<th width='15%'>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_LAST_QUERY_TIME, "</th>",
    "<th>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_STRING, "</th>",
    "<th width='100'>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_LAST_QUERY_RESULT_COUNT, "</th>",
    "<th width='200'>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_LAST_QUERY_USER, "</th>",
    "</tr>";
    ...
    foreach ($last_queries as $row) {
        ...
        "<td class='nobr'>", nc_search_util::format_time($row['Timestamp']), "</td>",
    ...
    "<table class='list'><tr>",
    "<th>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_STRING, "</th>",
    "<th width='150'>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_COUNT, "</th>",
    "<th width='150'>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_LAST_QUERY_RESULT_COUNT, "</th>",
    "</tr>\n";
...
if (count($rules)) {
    ...
    "<th>", NETCAT_MODULE_SEARCH_ADMIN_RULE_LAST_RUN, "</th>",
    "<th>", NETCAT_MODULE_SEARCH_ADMIN_STAT_INDEX, "</th>",
    "</tr>\n";
...
<script type="text/javascript">
    jQuery('#btn_schedule').click(function() {
        var area = jQuery('#area').val();
        if (area) { search_schedule(area); }
    });

    jQuery('#btn_now').click(function() {
        var area = jQuery('#area').val();
