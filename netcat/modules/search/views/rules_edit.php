...
?>
<fieldset>
    <legend><?= NETCAT_MODULE_SEARCH_ADMIN_RULE
?></legend>
  <?=$form->render("<div>", "", "</div>", "");
?>
    <div class="ncf_row space_before">
        <div class="ncf_caption"><?=NETCAT_MODULE_SEARCH_ADMIN_RULE_AREA_TO_INDEX
?>:</div>
        <div class="rule_radio">
            <!-- ↓ sic (ignored) -->
            <input type="radio" name="area" value="site" id="r-site" <?=($area ? "" : "checked")
?> />
            <label for="r-site"><?=NETCAT_MODULE_SEARCH_ADMIN_RULE_AREA_WHOLE_SITE
?></label>
        </div>
        <div class="rule_radio">
            <input type="radio" name="area" value="area" id="r-area" <?=($area ? " checked" : "")
?> />
            <label for="r-area"><?=NETCAT_MODULE_SEARCH_ADMIN_RULE_AREA_AREAS
?></label>
        </div>
        <div>
            <textarea id="r-area-input" rows="5" cols="80" name="data[area_string]" class="rule_area"><?=htmlspecialchars($area)
?></textarea>
        </div>
    </div>
    <div class="ncf_row space_before">
        <div class="ncf_caption"><?=NETCAT_MODULE_SEARCH_ADMIN_RULE_FREQUENCY
?>:</div>
        <!-- ( ) ежедневно в [  ]:[  ] -->
...
?> />
            <label for='r-req'><?=NETCAT_MODULE_SEARCH_ADMIN_RULE_INTERVAL_ON_REQUEST
?></label>
        </div>
