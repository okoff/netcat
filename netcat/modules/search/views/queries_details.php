...
echo "<div class='query_details_header'>",
 "<b>", sprintf(NETCAT_MODULE_SEARCH_ADMIN_QUERY_ALL_QUERIES, $query_string), "</b> (",
...
echo "<table class='list'>\n",
 "<tr>",
 "<th>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_TIME, "</th>",
 "<th width='40%'>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_AREA, "</th>",
 "<th>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_RESULTS_COUNT, "</th>",
 "<th>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_USER, "</th>",
 "<th>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_IP, "</th>",
 "</tr>\n";
...
foreach ($res as $row) {
    ...
    if (!$has_area) {
        $area_cell = "<td>$site_description</td>";
    } else {
        $area = new nc_search_area($row['Area'], $row['Catalogue_ID']);
        $description = array("included" => $area->get_description(false),
...
<script type='text/javascript'>
    (function($) {
        ...
        function() { $(this).find('.area_description').hide(); }
    );
    })($nc);
</script>