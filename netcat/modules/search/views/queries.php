    ...
    <table class="query_filter">
        <tr>
            <td class="fragment_cell">
                <div class="caption"><?=NETCAT_MODULE_SEARCH_ADMIN_QUERY_FRAGMENT
?></div>
                <input type="text" name="fragment" value="<?=$this->escape_input('fragment')
?>" class="fragment" />
            </td>
            <td id="timespan_inputs">
                <div class="caption"><?=NETCAT_MODULE_SEARCH_ADMIN_QUERY_TIME_PERIOD
?>
                    <span id="clear_timespan">[<a class="internal"><?=NETCAT_MODULE_SEARCH_ADMIN_QUERY_TIME_PERIOD_CLEAR
?></a>]</span>
                </div>
                <span class="timespan"><?=NETCAT_MODULE_SEARCH_ADMIN_QUERY_TIME_PERIOD_FROM
?> <?=$datetime_from
?></span>
                <span class="timespan"><?=NETCAT_MODULE_SEARCH_ADMIN_QUERY_TIME_PERIOD_TO
?> <?=$datetime_to
?></span>
            </td>
            <td>
                <div class="caption"><?=NETCAT_MODULE_SEARCH_ADMIN_QUERY_RESULTS
?></div>
                <select name="results">
                    <option value=""><?=NETCAT_MODULE_SEARCH_ADMIN_QUERY_RESULTS_ALL
?></option>
                    <option value="none"<?=($results == 'none' ? ' selected' : '')
?>><?=NETCAT_MODULE_SEARCH_ADMIN_QUERY_RESULTS_NONE
?></option>
                    <option value="matched"<?=($results == 'matched' ? ' selected' : '')
?>><?=NETCAT_MODULE_SEARCH_ADMIN_QUERY_RESULTS_MATCHED
?></option>
                </select>
            </td>
            <td class="submit">
...
<script type="text/javascript">
    (function($) {
        ...
        update = function() {
            ...
            has_value ? clear_link.show() : clear_link.hide();
        };
        inp.change(update);
        clear_link.click(function() { inp.val(''); clear_link.hide(); });
        update();
    })($nc);
</script>
...
if ($sort_by == 'time') {
    $order_by = "q.Timestamp DESC";
} elseif ($sort_by == 'query') {
    $order_by = "q.QueryString ASC";
}
...
$where[] = "Timestamp BETWEEN '".
        nc_search_util::sql_datetime($timestamp_from).
        "' AND '".
        nc_search_util::sql_datetime($timestamp_to).
        "'";
...
// это решение универсально, но может оказаться слишком медленным на
// больших объёмах информации:
$query = "SELECT SQL_CALC_FOUND_ROWS COUNT(`q`.`Query_ID`) as `QueryCount`, 
                   `q`.*,
                   `u`.`Login`
              FROM (SELECT *
                      FROM `Search_Query`
...
if ($res) {
    ...
    "<th rowspan='2' width='10%'>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_COUNT, "</th>",
    "<th colspan='3'>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_LAST_QUERY, "</th>",
    "</tr>\n",
    "<tr>",
    "<th>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_LAST_QUERY_TIME, "</th>",
    "<th>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_LAST_QUERY_RESULT_COUNT, "</th>",
    "<th>", NETCAT_MODULE_SEARCH_ADMIN_QUERY_LAST_QUERY_USER, "</th>",
    "</tr>\n";
