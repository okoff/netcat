...
foreach ($input_custom as $custom_field_settings) {
    ...
    foreach (array('is_retrievable', 'is_tokenized', 'is_sortable', 'is_indexed', 'is_stored') as $k) {
...
foreach ($fields as $name => $field) {
    if (preg_match("/^w\d+(?:_\d+)?$/", $name)) {
        if ($input_weight && !isset($saved_weight_fields[$name])) { // cleanup
            $field->delete();
        } else {
        ...
        if ($input_custom && !isset($saved_custom_fields[$name])) {
            $field->delete();
        } else {
...
<form method="POST" class="settings fields" onsubmit="return check_field_names();">
    ...
    <fieldset>
        <legend><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_DOCUMENT_AREAS
?></legend>
        <div class="setting">
            <div class="caption"><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_DOCUMENT_CONTENT
?>:</div>
            <div class="textarea">
                <textarea name="content[query]"><?=htmlspecialchars($fields["content"]->get('query'))
?></textarea>
            </div>
        </div>
        <div class="setting">
            <div class="caption"><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_DOCUMENT_NOINDEX
?>:</div>
            <div class="textarea">
                <textarea name="content[filter_content]"><?=htmlspecialchars($fields["content"]->get('filter_content'))
?></textarea>
            </div>
        </div>
    </fieldset>
...
?>' +
                ...
                '<a href="#" class="del internal"><?=addslashes(NETCAT_MODULE_SEARCH_ADMIN_SETTING_TAGS_DELETE)
?></a>' +
                '</div>';
            var last = 0;
            ...
            function add_tag_weight(index, field, focus) {
                var div = jQuery(tpl.replace(/\[x\]/g, "[" + index + "]")).appendTo('#tag_weight_container'),
                input = div.find("input.query");
                input.val(field.query);
                focus && input.focus();
                div.find("input.weight").val(field.weight);
                div.find("a.del").click(function() { jQuery(this).parent().remove(); return false; });
            }

            jQuery('#add_tag_weight').click(function() { add_tag_weight(last++, {weight: 1}, true)});
    ...
    <fieldset>
        <legend><?=NETCAT_MODULE_SEARCH_ADMIN_DATA_EXTRACTION
?></legend>
...
?>' +
                                     ...
                                     '<div class="settings_cb">' +
                                     '<input id="cb_tokenized_x" name="custom[x][is_tokenized]" type="checkbox" value="1" /> ' +
                                     '<label for="cb_tokenized_x">' +
                                     '<?=htmlspecialchars(NETCAT_MODULE_SEARCH_ADMIN_DATA_EXTRACTION_FIELD_IS_TOKENIZED)
?>' +
                                     '</label>' +
                                     '</div>' +
...
?>' +
                                 ...
                                 '<div class="remove_custom_field">' +
                                     '<a href="#" class="del internal"><?=htmlspecialchars(NETCAT_MODULE_SEARCH_ADMIN_SETTING_TAGS_DELETE)
?></a>' +
                                     '</div>' +
                                     '</div>';
                                 var last = 0;
                                 ...
                                 function add_custom_field(index, field, focus) {
                                     var div = jQuery(tpl.replace(/\[x\]|_x\b/g, "[" + index + "]")).appendTo('#custom_field_container');

                                     jQuery.each(field, function(name, value) { // set values
                                         var el = div.find("[name='custom[" + index + "][" + name + "]']").first();
                                         if (el.attr('type') == 'checkbox') { el.attr('checked', (value == true ? 'checked' : '')); } // sic!
                                         else { el.val(value); }
                                     });
                                     ...
                                     div.find("a.del").click(function() { jQuery(this).parent().parent().remove(); return false; })
                                     .hover(function() { jQuery(this).parent().parent().addClass('custom_field_delete_hover')},
                                     function() { jQuery(this).parent().parent().removeClass('custom_field_delete_hover')});
                                 }

                                 jQuery('#add_custom_field').click(function(e) {
...
?>);
                             ...
                             function check_field_names() {
                                 ...
                                 jQuery('input.custom_field_name').each(function() {
                                     ...
                                     is_invalid =
                                         /(\W+)|^(title|content|doc_id|site_id|sub_id|ancestor|language|last_modified|access_level|w[\d_]+)$/
                                     .test(input.val())
                                         || !input.val();
