...
?>
    ...
    <div style="margin-left: 20px;">
        <script type="text/javascript">
            ...
            (function() {
                var toggle = function() {
                    var enabled = jQuery('#cb_EnableSearch').attr('checked');
                    jQuery('#search_module_settings input, #search_module_settings select, #search_module_settings textarea')
                    .attr('disabled', !enabled);
                }
                jQuery('#cb_EnableSearch').click(toggle);
                jQuery(toggle);
            })();
        </script>
        <div id="search_module_settings">
            <fieldset>
                <legend><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_INDEXING
    ?></legend>
                <div class="setting">
                    <div class="caption"><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_DISABLE_SECTION_INDEXING
    ?>:</div>
                    <div class="textarea"><textarea name="s[ExcludeUrlRegexps]"><?=htmlspecialchars(nc_search::get_setting('ExcludeUrlRegexps'))
    ?></textarea></div>
                </div>
                <div class="setting">
    ...
    ?>
        ...
        <?=$this->setting_cb('RemoveStopwords', sprintf(
                                NETCAT_MODULE_SEARCH_ADMIN_SETTING_USE_STOPWORDS, $this->hash_href("#module.search.stopwords")
            ...
            <?=sprintf(NETCAT_MODULE_SEARCH_ADMIN_SETTING_MINIMUM_WORD_LENGTH,
                            ...
                            ($shortwords_filter && $shortwords_filter->get('enabled') ? htmlspecialchars(nc_search::get_setting('MinWordLength')) : 1
                            )."'>")
    ?>
                </div>
    ...
    ?>
                ...
                <script type="text/javascript">
                    ...
                    (function() {
                        var toggle = function() {
                            var enabled = jQuery('#cb_TryToCorrectQueries').attr('checked');
                            jQuery('#correction_options input').attr('disabled', !enabled);
                        }
                        jQuery('#cb_TryToCorrectQueries').click(toggle);
                        toggle();
                    })();
                </script>
            </fieldset>
            ...
            <fieldset>
                <legend><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_QUERY_LOG
    ?></legend>
                <?=$this->setting_cb('SaveQueryHistory', NETCAT_MODULE_SEARCH_ADMIN_SETTING_QUERY_LOG_ENABLED)
    ?>
                        <div class="subgroup">
                            <div class="caption"><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_QUERY_LOG_PURGE ?>:</div>
                    <blockquote>
                        <div class="setting">
                            <input type="hidden" name="s[AutoPurgeHistoryIntervalValue]" value="0" />
                            <input type="radio" name="s[AutoPurgeHistory]" value="0" id="r_ph0"
                    <?=(!$log_purge_interval ? " checked" : "") ?>/>
                            <label for="r_ph0"><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_QUERY_LOG_PURGE_NEVER
    ?></label>
                        </div>
                        <div class="setting">
                            <input type="radio" name="s[AutoPurgeHistory]" value="1" id="r_ph1"
                    <?=($log_purge_interval ? " checked" : "") ?>/>
                            <label for="r_ph1"><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_QUERY_LOG_PURGE_BEFORE ?></label>
                            <input type="text" name="s[AutoPurgeHistoryIntervalValue]" class="i3"
                                id="purge_interval"
                        ...
                        <script type="text/javascript">
                            ...
                            jQuery('#r_ph0').click(function() { jQuery('#purge_interval').val(''); });
                            // обработка ввода в поле «раньше...»
                            jQuery('#purge_interval').keyup(function() {
                                var radio = (jQuery(this).val() ? '#r_ph1' : '#r_ph0');
                ...
                <div class="subgroup">
                    <div class="caption">
                        <a class="internal" href="#" onclick="jQuery('#purge_now').toggle(300); return false"><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_QUERY_LOG_PURGE_NOW
                                ?></a>
                    </div>
                    <blockquote id="purge_now">
                        <div class="setting">
                            <input type="radio" name="purge[interval]" value="-1" id="r_pnow0"
                    <?=(!$log_purge_interval ? " checked" : "")
                                ?>/>
                            <label for="r_pnow0"><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_QUERY_LOG_PURGE_NOW_EVERYTHING ?></label>
                        </div>
                        <div class="setting">
                            <input type="radio" name="purge[interval]" value="1" id="r_pnow1" />
                            <label for="r_pnow1"><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_QUERY_LOG_PURGE_BEFORE
                                ?></label>
                            <input type="text" name="purge[interval_value]" class="i3" id="purge_now_interval" />
    ...
    ?>
                        ...
                        <script type="text/javascript">
                            ...
                            jQuery('#r_pnow0').click(function() { jQuery('#purge_now_interval').val(''); });
                            // обработка ввода в поле «раньше...»
                            jQuery('#purge_now_interval').keyup(function() {
                                var radio = (jQuery(this).val() ? '#r_pnow1' : '#r_pnow0');
                                jQuery(radio).attr('checked', 'checked');
                            });
                        </script>
                        <div class="submit_button">
                            <input type="submit" name="purge[now]" title="<?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_QUERY_LOG_PURGE_NOW_SUBMIT ?>"  value="<?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_QUERY_LOG_PURGE_NOW_SUBMIT ?>" />
                                    </div>
                                </blockquote>
                        </fieldset>
                    ...
                    <select name="s[DefaultBooleanOperator]">
                        <option value="AND"><?=NETCAT_MODULE_SEARCH_ADMIN_SETTING_QUERY_DEFAULT_OPERATOR_AND
    ?></option>
    ...
    ?>
            ...
            <script type="text/javascript">
                ...
                jQuery('a.internal.on_page').click(function() {
                    var name = jQuery(this).attr('href').substr(1),
                    target = jQuery("a[name='" + name + "']").next('div').find('label');
                    // can't animate background color without jQuery UI
                    // (not available here at the time of creation of this code)
                    jQuery('body').animate({scrollTop: target.offset().top-100}, 500);
                    setTimeout(function() {
