<?php
/* $Id: cron.php 3127 2009-04-10 08:09:00Z vadim $ */
...
if ($MODULE_VARS['search']['DISALLOWED_URLS']) $disallowed_urls = explode(",",$MODULE_VARS['search']['DISALLOWED_URLS']);
if ($MODULE_VARS['search']['DISALLOWED_REGEXP'])  { $disallowed_regexp = explode (", ",(get_magic_quotes_gpc()?stripslashes($MODULE_VARS['search']['DISALLOWED_REGEXP']):$MODULE_VARS['search']['DISALLOWED_REGEXP']) ); }
$index_table = intval($MODULE_VARS['search']['INDEX_TABLE']);
...
while ($url) {
	...
	if ($count && $count%$MODULE_VARS['search']['PAGES_COUNT']==0 ) echo "<script type='text/javascript' language='JavaScript'>\nwindow.location.href='".$SUB_FOLDER.$HTTP_ROOT_PATH."modules/search/cron.php?param=".$param."&repeat=".($repeat+$MODULE_VARS['search']['PAGES_COUNT'])."';\n</script>";
	$count++;
