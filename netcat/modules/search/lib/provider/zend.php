...
/* $Id: zend.php 8454 2012-11-23 09:21:46Z aix $ */
...
class nc_search_provider_zend implements nc_search_provider {
     ...
     */
    protected function open_index($create_if_not_exists = true) {
    ...
    /**
     * Приведение строки запроса к верхнему регистру, исключая имена полей.
     * Необходимо для того, чтобы результаты поиска были верными.
     * @param string $query_string
     * @return string
     */

    private function normalizeQueryCase($query_string) {
        // Fix possible spaces between : and field name
        $query_string = preg_replace('/\s:/', ':', $query_string);
        $query_string = preg_replace('/:\s/', ':', $query_string);

        // Find all field names
        preg_match_all('/[a-zA-Z0-9]*:/ui', $query_string, $field_names);
        $query_string = mb_strtoupper($query_string, 'UTF-8');

        foreach($field_names as $arr) {
            foreach($arr as $a) {
                $query_string = preg_replace('/' . mb_strtoupper($a, 'UTF-8') . '/u', $a, $query_string);
            }
        }
        return $query_string;
    }
    ...
    protected function get_lucene_query(nc_search_query $query) {
        ...
        $zend_query = Zend_Search_Lucene_Search_QueryParser::parse($this->normalizeQueryCase($query_string), 'UTF-8');
     ...
     * @param nc_search_document $document
     */
    public function add_document(nc_search_document $document) {
        $document->save();
        $this->set_indexing_context($document);
     ...
     * @param nc_search_document $document
     */
    public function update_document(nc_search_document $document) {
        $this->mark_as_deleted($document);
    ...
    public function process_document(nc_search_document $document) {
        ...
        $stored = nc_search_document::get_by_path($document->get('site_id'), $document->get('path'));

        if ($stored) {
            // Нужно ли обновлять? Проверить хэш для content
    ...
    public function suggest_titles($input, $language, $site_id) {
        ...
        $suggestions = (!$nc_core->NC_UNICODE ? $nc_core->utf8->array_win2utf($suggestions) : $suggestions);
