...
class nc_search_result extends nc_search_data_persistent_collection {
    ...
    protected $data_class = 'nc_search_result_document';
    /**
     * @var nc_search_query
     */
    protected $query;
     ...
     * @param nc_search_result_document $item
     */
    ...
    protected function highlight($text, $language, $max_num_words) {
        ...
        //   (предположительно — сокращенее, инициал), его следует «прилепить»
        //   к текущему;
    ...
    public function convert() {
        foreach ($this->items as $item) {
            foreach (array('title', 'context', 'content', 'meta') as $v) {
                $item->set($v, nc_search_util::convert($item->get($v)));
            }
        }
    }
