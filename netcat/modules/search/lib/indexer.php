...
class nc_search_indexer {
    ...
    public function finalize($is_cancelled = false) {
        ...
        if ($is_cancelled) {
            ...
            $to_delete = nc_search::load('nc_search_document', "SELECT `Document_ID` FROM `%t%` WHERE `ToDelete` = 1");
            $task->set('total_deleted', $to_delete->count());
            foreach ($to_delete as $doc) {
    ...
    public function next() {
        ...
        if ($response) {
            ...
            } elseif ($response->has_body()) { // есть ответ
    ...
    /**
     * Обработать ответ ползователя (crawler)
    ...
    protected function process_response(nc_search_indexer_crawler_response $response, nc_search_indexer_link $link) {
        ...
        if ($response->get_body_length() > ini_get('pcre.backtrack_limit')) {
            if (nc_search::will_log(nc_search::LOG_INDEXING_CONTENT_ERROR)) {
                nc_search::log(nc_search::LOG_INDEXING_CONTENT_ERROR, 'Received length of content ('.$response->get_body_length().') exceeded the permissible value (pcre.backtrack_limit = '.ini_get('pcre.backtrack_limit').')');
            }
            return false;
        }
        ...
        foreach ($page_link_ids as $page_link_id) {
            $this->referrer_cache[] = array($doc_id, $referrer_link_id, $page_link_id);
        }
    }
    ...
    public static function get_current_task() {
        ...
        // не подвисли ли мы?
        $task = $tasks->first();
        if (time() > ($task->get('last_activity') + nc_search::get_setting("IndexerRemoveIdleTasksAfter"))) {
            $task->delete();
    ...
    protected function flush_referrer_cache() {
        if (!sizeof($this->referrer_cache)) {
            return;
        }
        $values = json_encode($this->referrer_cache);

        if (stripos($values, "null") !== FALSE) {
            return;
        }
        $values = strtr($values, "[]", "()");
        $values = substr($values, 1, -1); // remove brackets from the start and the end
