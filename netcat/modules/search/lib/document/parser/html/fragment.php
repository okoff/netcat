...
class nc_search_document_parser_html_fragment {
    ...
    protected function set_html($html) {
        // Для корректного парсинга нужен заголовок с указанием кодировки (указания
        // в конструкторе недостаточно). К счастью, DOMDocument великодушно прощает
        // столь вольное обращение с HTML:
        $meta = "<meta http-equiv='content-type' content='text/html; charset=utf-8' />";
        $html .= "\n";
        $this->dom->loadHTML("$meta<nc-fragment>$html</nc-fragment>\n");
    }
    ...
    public function append_nodes(DOMNodeList $nodes) {
        $dom = $this->dom;
        $fragment_node = $dom->createElement('nc-fragment');
        $dom->appendChild($fragment_node);
        $this->append_nodes_to($fragment_node, $nodes);
    }

    /**
     * Получить узлы с контентом
     * @return DOMNodeList
     */
    public function get_nodes() {
        return $this->dom->getElementsByTagName('nc-fragment');
    }
    ...
    public function get_html() {
        preg_match_all("#<nc-fragment>(.*?)</nc-fragment>#s",
                $this->dom->saveHTML(),
                $matches);
        return trim(join("\n", $matches[1]));
    }
     ...
     * @param nc_search_document_parser_html_part|null $recipient
    ...
    protected function extract_nodes($nodes, self $recipient = null, $remove = false) {
        ...
        if ($recipient) {
            $recipient->append_nodes($nodes);
        } // must be done before removing the nodes ↓
        if ($remove) {
    ...
    public function extract_regexp($regexp, self $recipient = null, $remove = false) {
        $html = $this->get_html();
        if (!nc_preg_match_all($regexp, $html, $matches)) {
            return false;
        }
        if ($recipient) {
