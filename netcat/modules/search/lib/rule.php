...
//      'start_url' => '/',
    ...
    protected $serialized_options = array('last_result');
    ...
    public function schedule_next_run() {
        ...
        $where = $intent->make_condition('area_string', $this->get_id(),
                        'type', $type_scheduled);
        $this->get_db()->query("DELETE FROM `{$intent->get_table_name()}` WHERE $where");
    ...
    public function save() {
        if (!$this->get('interval_type')) {
            throw new nc_data_exception(NETCAT_MODULE_SEARCH_ADMIN_RULE_MUST_HAVE_INTERVAL_TYPE);
        }
    ...
    public function delete() {
        ...
        $db->query("DELETE FROM `Search_Schedule`".
                " WHERE `AreaString` = '".$db->escape($this->get_id())."'");
