...
class nc_subscriber_send {
 ...
 // номера подписок, которые участвовали в рассылке
     ...
     */
    public function send_mail($to, $body, $subject, $isHtml, $files = array()) {
        static $mailer;
        static $filetable;
    ...
    public function send() {
        ...
        foreach ($users as $user_id => $cc_messages) {
            ...
            if ($this->_settings['MergeMail']) {
                ...
                $this->send_mail($current_user[$this->_settings['EmailField']], $m, $mail_subject[0], $isHtml);
            }
            else {
                for ($i = 0; $i < $cc_num; $i++) {
                    if ($mail_not_send[$i]) continue;
                    ...
                    $this->send_mail($current_user[$this->_settings['EmailField']], $m, $mail_subject[$i], $isHtml);
                    $this->_mailer_count[$cc_mailer[$cc_array[$i]]]++;
                    $this->_stats_full_update($cc_mailer[$cc_array[$i]], $user_id);
                }
            }
    ...
    public function send_periodical() {
        ...
        foreach ($users as $user_id => $mailers) {
            ...
            foreach ($mailers as $mailer_id) {
                ...
                $this->send_mail($current_user[$this->_settings['EmailField']], $m, $mail_subject, $isHtml);
    ...
    public function send_prepared() {
        ...
        $mails = $this->db->get_results("SELECT `ID`, `Email`, `Subject`, `Body`, `HTML`, `Files`
        ...
        foreach ($mails as $mail) {
            ...
            $this->send_mail($mail['Email'], $mail['Body'], $mail['Subject'], $mail['HTML'], $files);
            $delete_id[] = $mail['ID'];
        }
        ...
        while (1) {
            ...
            foreach ($this->users_id as $user_id) {
                ...
                foreach ($res as $row) {
                    ...
                    $this->db->query("INSERT INTO `Subscriber_Prepared` ( `Email`, `Subject`, `Body`, `HTML`)
                          VALUES('".$current_user[$this->_settings['EmailField']]."','".$mail_subject."','".$m."','".$isHtml."')");
                    $this->_mailer_count[$mailer_id]++;
                    $this->_stats_full_update($mailer_id, $user_id);
                }
            }
