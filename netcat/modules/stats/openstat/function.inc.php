...
function nc_openstat_put_counter_to_templates($check_xml = 0, $put_templ_ids=NULL) {
    ...
    if (!$put_templ_ids) {
        ...
        $templates = $db->get_results("SELECT `Template_ID`, `Parent_Template_ID`, `Header`, `Footer` FROM `Template` WHERE `Template_ID` IN (".$where_in.")");
    }
    if (!$templates) {
        return 1;
    }
...
function nc_openstat_get_code() {
    ...
    if (!$nc_openstat_already_set && ($nc_core->get_settings('Openstat_Enabled', 'stats') == 1)) {
        ...
        if ($counter_code) {
            return addslashes($counter_code);
        }
    }
}
