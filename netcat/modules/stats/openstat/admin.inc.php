...
function write_template($ParentTemplateID, &$NoCounterTemplates, $count = 0) {
    ...
    $Result = $db->get_results("SELECT Template_ID,Description FROM `Template` where `Parent_Template_ID`='".$ParentTemplateID."' AND `Template_ID`<>'".$nc_core->get_settings('EditDesignTemplateID')."' ORDER BY Template_ID", ARRAY_N);
    if ($Result) {
        ...
        <td class='withBorder' style='padding-left:".intval($count * ($count == 1 ? 15 : 20) + 5)."px;".(!$ParentTemplateID ? " font-weight: bold;" : "")."'>".($ParentTemplateID ? "<img src='".$ADMIN_PATH."images/arrow_sec.gif' border='0' width='14' height='10' alt='arrow' title='".$Array[0]."'>" : "")."<span>".$Array[0].". </span><a target='_blank' href='".$ADMIN_PATH."/#template.edit(".$Array[0].")'><font  ".($NoCounterTemplates[$Array[0]] ? "color='#cccccc'" : "").">".$Array[1]."</font></a></td>";
            $res.= "</tr>";
            $res.= "</table>";
            // children
...
function show_counters($phase) {
    ...
    if ($counters) {
        ...
        $UI_CONFIG->actionButtons[] = array("id" => "delete_all",
                "align" => "left",
