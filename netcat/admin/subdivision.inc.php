...
function DeleteFromSubClass($SubdivisionID) {
    ...
    if (!empty($res)) {
        $CatalogueID = $res[0]['Catalogue_ID'];
        foreach ($res as $val) {
            DeleteSubClassFiles($val['Sub_Class_ID'], $val['Class_ID']);
            $subclass_to_delete_arr[] = $val['Sub_Class_ID'];
        }
    }

    if (nc_module_check_by_keyword("comments")) {
        if (!empty($subclass_to_delete_arr)) {
            include_once ($MODULE_FOLDER."comments/function.inc.php");
            // delete comment rules
            nc_comments::dropRuleSubClass($db, $subclass_to_delete_arr);
            // delete comments
            nc_comments::dropComments($db, $subclass_to_delete_arr, "Sub_Class");
        }
    }

    $db->query("DELETE FROM `Sub_Class` WHERE `Subdivision_ID` = '".(int) $SubdivisionID."'");

    if ($CatalogueID && $subclass_to_delete_arr) {
        $nc_core->event->execute("dropSubClass", $CatalogueID, $SubdivisionID, $subclass_to_delete_arr);
    }
