...
function ClassificatorList($IsSystem) {
    ...
    if ($countClassif = $db->num_rows) {
        ...
        <? if ($perm->isAccess(NC_PERM_CLASSIFICATOR, NC_PERM_ACTION_DEL)) : ?>
								<td align='center'>
									<div class='icons icon_settings' title='<?= CONTROL_CONTENT_CATALOUGE_FUNCS_SHOWCATALOGUELIST_DELETE ?>'></div>
								</td>
                            <? endif; ?>
                            </tr>
                            <?
...
function AddClassificator($ClassificatorName, $ClassificatorTable, $System, $SortType = 0, $SortDirection = 0) {
    ...
    ?>
        <form method=post action="classificator.php">
            ...
            <?= CONTENT_CLASSIFICATORS_ADD_KEYWORD ?>:<br><?= nc_admin_input_simple("ClassificatorTable", $ClassificatorTable, 50, '', "maxlength='32'") ?><br><br>
            <?= CONTROL_CONTENT_CATALOUGE_FUNCS_CATALOGUEFORM_NAME
            ?>:<br><?= nc_admin_input_simple("ClassificatorName", $ClassificatorName, 50, '', "maxlength='32'")
        ?><br><br>
...
function AddClassificator_modal() {
    ...
    <form method='post' id='adminForm' action="classificator.php">
...
function UpdateClassificatorCompleted($ClassificatorID, $ClassificatorName, $System, $SortType, $SortDirection) {
    ...
    if ($ClassificatorName == "") {
        print "" . CONTENT_CLASSIFICATORS_ERROR_NAME . "<br>\n";
        OneClassificatorList($ClassificatorID, $SortType, $SortDirection);
    } else {
...
function OneClassificatorList($ClassificatorID, $SortType, $SortDirection) {
    ...
    if (!$admin_cl):  //Есть доступа к измененению названия списка..?
        ...
        <table cellspacing='3' cellpadding='3'>
            ...
            <tr>
                <td><?= CONTROL_SCLASS_LISTING_NAME
        ?>:</td><td><?= $Name
                ?></td>
            </tr>
        </table>
    ...
    <? else : ?>
        ...
        $Sort_Order = " ORDER BY ";
        switch ($SortType) {
            case 1: $Sort_Order .= "${TableName}_Name";
                break;
            case 2: $Sort_Order .= "${TableName}_Priority";
                break;
            default: $Sort_Order .= "${TableName}_ID";
                break;
        }
        ...
        <? if ($admin_cl) : ?>
								<td class='align-center'><div class='icons icon_type_bool' title='<?= CONTROL_CONTENT_CATALOUGE_FUNCS_CATALOGUEFORM_ON ?>'></div></td>
								<td class='align-center'><div class='icons icon_prior' title='<?= CLASSIFICATORS_SORT_PRIORITY_HEADER ?>'></div></td>
        ...
        <? endif; ?>
                            ...
                            foreach ($Result as $Array) {
                                print "<tr>";
                                print "<td>" . $Array[0] . "</td>";
                                print "<td><a onclick='parent.nc_form(this.href); return false;' href={$ADMIN_PATH}classificator.php?phase=10&ClassificatorID=" . $ClassificatorID . "&IdInClassificator=" . $Array[0] . ">" . $Array[1] . "</a></td>";
                                if ($admin_cl) {
...
function DeleteFromOneClassificator($ClassificatorID, $IdInClassificator) {
    ...
    if (!IsSystemClassificator($ClassificatorID)) {
        ...
        print(CONTENT_CLASSIFICATORS_ERR_SYSDEL);
    }
}
...
function InsertInOneClassificator($ClassificatorID) {
    ...
    <form method='post' action='classificator.php'>
        ...
        <table cellpadding='0' cellspacing='0'>
            <tr>
                <td>
                    <?= CONTENT_CLASSIFICATORS_ELEMENT_NAME
    ?>:<br>
    <?= nc_admin_input_simple('NameInClassificator', '', 50, '', "maxlength='256'") ?>
                </td>
                <td width='4'>&nbsp;</td>
                <td>
                    <?= CLASSIFICATORS_SORT_PRIORITY_HEADER
    ?>:<br>
    <?= nc_admin_input_simple('Priority', $LastPriority, 6, '', "maxlength='5'") ?>
                </td>
            </tr>
            <tr>
                <td>
    <?= nc_admin_textarea_simple('ValueInClassificator', '', "" . CONTENT_CLASSIFICATORS_ELEMENT_VALUE . ":<br>", 7, 0, '', 'soft') ?>
                </td>
                <td colspan='2' />
            </tr>
        </table>
...
function UpdateOneClassificator($ClassificatorID, $IdInClassificator) {
    ...
    print "<FORM METHOD=\"POST\" ACTION=\"classificator.php\">\n";
...
function UpdateOneClassificator_modal($ClassificatorID, $IdInClassificator) {
    ...
    echo "<form method='post' id='adminForm' action='classificator.php'>";
    ...
    echo "
       ...
       "<div>" . nc_admin_input_simple('NameInClassificator', $Array[0], 0, 'width: 30%;', "maxlength='256'") . "</div>
        <div>" . nc_admin_textarea_simple('ValueInClassificator', $Array[1], "<div>" . CONTENT_CLASSIFICATORS_ELEMENT_VALUE . ":</div>", 7, 0, "style='width: 30%;'", 'soft') . "</div>
...
function ImportClassificator($ClassificatorName, $ClassificatorTable, $System, $SortType = 0, $SortDirection = 0) {
    ...
    <form enctype='multipart/form-data' action=classificator.php method=post>
        ...
        <?= CONTROL_CONTENT_CATALOUGE_FUNCS_CATALOGUEFORM_NAME ?>:<br><?= nc_admin_input_simple('ClassificatorName', $ClassificatorName, 50, '', "maxlength='32'") ?><br><br>
...
function ImportClassificator_modal() {
    ...
    ?>
            ...
            <form id='adminForm' enctype='multipart/form-data' action=classificator.php method=post>
                <input type=hidden name=MAX_FILE_SIZE value=1000000>
                <input type=hidden name=phase value=13>
