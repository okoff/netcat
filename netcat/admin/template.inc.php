...
function BeginHtml($title = "", $location = "", $HelpURL = "", $module = '', $developer_mode = false) {
    ...
    if (!$developer_mode) {
        ?><!DOCTYPE html>
          <html lang='<?= MAIN_LANG ?>' dir='<?= MAIN_DIR ?>'>
            <head>
                ...
                <script type='text/javascript' src='<?= $ADMIN_PATH ?>js/chosen.jquery.js?<?= $LAST_LOCAL_PATCH ?>'></script>
                <script type="text/javascript">
                    $nc(".chzn-select").chosen();
                    $nc(".chzn-select-deselect").chosen({allow_single_deselect:true});
            ...
            <body<? } else { ?><div<? } ?> class='admin_form' id='MainViewBody'>
                <?
            }
...
<script src='<?=$ADMIN_PATH?>js/codemirror/lib/simple-hint.js'></script>
<script src='<?=$ADMIN_PATH?>js/codemirror/lib/netcat-hint.js'></script>
<script src='<?=$ADMIN_PATH?>js/codemirror/lib/cm_init.js'></script>
<script type='text/javascript'>
	var nc_cmConfig = {
		CMAutocomplete:!!'<?=$set['CMAutocomplete']?>',
		CMHelp:!!'<?=$set['CMHelp']?>',
		CMDefault:!!'<?=$set['CMDefault']?>',
		autoCompletionData: $nc.parseJSON("<?=addslashes(json_encode(get_autocompletion_data()))?>"),
