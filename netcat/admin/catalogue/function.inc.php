...
function CatalogueForm($CatalogueID, $phase, $action, $type, $bar_action) {
    # type = 1 - это insert
    # type = 2 - это update
    ...
    echo "<form id='adminForm' enctype='multipart/form-data' method='post' name='adminForm' action='" . $action . "'>";
    ...
    $display = array (
            'edit' => $bar_all || $bar_action == 'edit',
            'design' => $bar_action == 'design',
            'seo' => $bar_action == 'seo',
            'system' => $bar_action == 'system',
            'fields' => $bar_action == 'fields');
    ...
    $fieldsets->new_fieldset('template', CONTROL_CONTENT_CATALOUGE_FUNCS_CATALOGUEFORM_TEMPLATE)->add(nc_subdivision_form_design($Array, $CatalogueID))->show($display['design']);
   ...
   " . nc_admin_textarea_simple('Robots', ($Array["Robots"] ? $Array["Robots"] : "# NetCat 4 Robots file\nUser-agent: *\nDisallow: /install/"),CONTROL_CONTENT_CATALOUGE_FUNCS_CATALOGUEFORM_ROBOTS, 4, 10) . "<br /><br />
    ...
    $fieldsets->mobile->add(ob_get_clean());
...
function ActionCatalogueCompleted($CatalogueID, $type) {
    ...
    if ($type == 1) {
        ...
        $insert.= "'" . $db->escape($nc_core->input->fetch_get_post('language')) . "',";
...
function checkDomain($domain, $CatalogueID) {
    $error = '';
    $uniqueFileUrl = createUniqueFile();

    switch (true) {
        case !isFieldSet($domain):
            $error = ' ('.CONTROL_CONTENT_CATALOUGE_ERROR_DOMAIN_NOT_SET.')';
            break;

        case !$uniqueFileUrl:
            $error = ' ('.NETCAT_ADMIN_NOTICE_RIGHTS.' '.nc_Core::get_object()->HTTP_FILES_PATH.')';
            break;

        case !isDomainCorrect($domain, $uniqueFileUrl):
            $error = " (<a style='vertical-align: top;' href='".nc_Core::get_object()->ADMIN_PATH."catalogue/index.php?phase=2&type=2&CatalogueID=$CatalogueID'>".CONTROL_CONTENT_CATALOUGE_ERROR_INCORRECT_DOMAIN."</a>)";
