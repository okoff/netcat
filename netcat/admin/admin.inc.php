...
function nc_array_json($array) {
    ...
    foreach ($array as $k => $v) {
        ...
        if ($array_is_numeric) {
            ...
            $ret_values[] = (is_numeric($k) ? $k : "\"".addcslashes($k, '"')."\"").": ".$v;
        }
    }
...
?>
		<div id='statusMessage' class='nc_print_status status_<?=$status?>'>
			<div class='nc_print_status_icon nc_print_status_icon_<?=$status?>'></div>
			<div class='nc_print_status_text'><?=$message?></div>
...
class ui_subdivision_data {
    ...
    function fetch_by_subdivision_id($sub_id) {
        ...
        if (!$this->subdivision_id) {
            global $db;
            $sub_data = $db->get_row("SELECT Subdivision_ID, Subdivision_Name, Catalogue_ID, Hidden_URL, Checked
                                  FROM Subdivision
            ...
            $this->subdivision_url = $sub_data['Hidden_URL'];
    ...
    function fetch_by_query($qry) {
        ...
        if ($this->subclasses[0]) {
            ...
            $this->subdivision_url = $this->subclasses[0]['Hidden_URL'];
 ...
 */
class ui_config_subdivision_gereric extends ui_config {
    ...
    function init($sub_id, $cc_id=0) {
        ...
        if ($sub_id) {
            ...
            trigger_error("Wrong parameters for ui_config_subdivision_gereric::init", E_USER_ERROR);
        }
        ...
        if (false && $this->sub->is_subdivision_admin()) {
        ...
        $this->tabs[] = array('id' => 'view',
                ...
                'action' => "window.open('http://" . nc_Core::get_object()->catalogue->get_by_id($this->sub->catalogue_id, 'Domain') . $this->sub->subdivision_url."');"
        );
...
class ui_config_objects extends ui_config_subdivision_gereric {

    function ui_config_objects($cc_id) {
        $this->init(0, $cc_id);
        if (!$cc_id) {
 ...
 * Использование: ob_start("nc_postprocess_admin_page").
 * Эффекты:
...
function nc_postprocess_admin_page($str) {
    ...
    global $inside_admin, $HTTP_ROOT_PATH, $SUB_FOLDER;
    if (!$inside_admin) return $str;
    // current adress
    global $parsed_url;

    $local_links_re = "(                                                  # $1: whole link
		(                               			                              # $2: script path
			(?:                           			                              # group: absolute path
				(?:                                                             #
					".preg_quote($parsed_url['scheme']).":\/\/                    # protocol
					".preg_quote(str_replace("www.", "", $parsed_url['host']))."  # host
					(?::".preg_quote($parsed_url['port']).")? 	                  # port
				)?                         			                                # host part is optional
				".preg_quote($SUB_FOLDER.$HTTP_ROOT_PATH)." 	                  # /netcat/
				(?![a-z]+\/)                 			                              # but not /netcat/dirname/
			)                             			                              # optional (relative path)
			(?:[^\/\?\#'\"\s>]*)?        			                                # script name (optional)
		)                               			                              # grab path
		(?:\?([^'\"\s>]+))?            				                              # followed by GET params ($3)
	)";

    // look for forms and hrefs
    nc_preg_match_all("@
    <(?:a|form) \s
    [^>]*
    (?:href|action)\s*=\s*[\"']?".$local_links_re."[\"']?
    [^>]*
    >
    @xs", $str, $regs, PREG_PATTERN_ORDER);

    // regs[1] - full link; [2] path; [3] get params

    foreach ($regs[3] as $num => $get_params) {
        if (!$get_params || !preg_match("/[?&]inside_admin=/", $get_params)) {
            $new_link = str_replace($regs[1][$num],
                            $regs[2][$num]."?inside_admin=".$inside_admin.($get_params ? "&amp;".$get_params : ""),
                            $regs[0][$num]);
            $str = str_replace($regs[0][$num], $new_link, $str);
        }
    }
...
//  if ($dest_subclass['Sub_Class_ID'] == $message['Sub_Class_ID']) { return true; } // Проверка на копирование объекта внутри одного $cc
    ...
    // Если полей нет, то можно завершать работу функции
    if (empty($file_fields)) return $new_message_id;
    ...
    foreach ($file_fields as $field) {
        ...
        switch ($fs) {
            ...
            case NC_FS_ORIGINAL:
                ...
                if (file_exists($FILES_FOLDER.$put_file_name)) {
                    while (file_exists($FILES_FOLDER.substr($put_file_name, 0, strrpos($put_file_name, "."))."_".$k.$ext))
                        $k++;

                    $put_file_name = substr($put_file_name, 0, strrpos($put_file_name, "."))."_".$k.$ext;
                }
                $FilePath = ''; // в папку netcat_files
                $fieldValue = $src_file_name.":".$src_file_type.":".$src_file_size.":".$put_file_name; // значение в базу
                break;
...
function nc_admin_textarea($disc, $name, $value, $is_resizeble = 0, $fck = 0, $style = '') {
    ...
    if ($fck) {
        $ret .= "<button type='button' onclick=\"window.open('".$nc_core->SUB_FOLDER.$nc_core->HTTP_ROOT_PATH."editors/FCKeditor/neditor.php?form=main&control=".$name."', 'Editor', 'width=750,height=605,resizable=yes,scrollbars=no,toolbar=no,location=no,status=no,menubar=no');\">".TOOLS_HTML_INFO."</button><br>";
    }
    if ($is_resizeble) {
...
function nc_admin_select_subdivision($disc, $name, $value) {
    ...
    $res .= "<select class='chzn-select' name='".$name."'>".nc_select_options($subs, $value)."</select></div>";
    return $res;
}
...
function nc_admin_select_component($disc, $name, $value = '') {
    ...
    $res = "<div style='margin:5px 0; _padding:0;'>".$disc." ";
    $res .= "<select class='chzn-select' name='".$name."'>".nc_select_options($classes, $value)."</select></div>";
    return $res;
}
...
function nc_admin_select_field($table, $disc, $name, $value = '') {
    ...
    $res = "<div style='margin:5px 0; _padding:0;'>".$disc." ";
    if (!empty($fields)) {
        $res .= "<select class='chzn-select' name='".$name."'>\n";
        foreach ($fields as $v) {
...
function nc_customsettings_show($ClassID = 0, $TemplateID = 0, $custom_settings = array(), $Class_Template = 0) {
    ...
    $header = " <table class='admin_table' style='width: 100%;'>
            <tr>
    ...
    echo $a2f->settings_render($header, '<tr style="background-color: #FFF;"><td><a href="index.php?phase='.($ClassID ? ($Class_Template ? 250 : 25) : 9).'&amp;'.( $ClassID ? "ClassID=".$ClassID : "TemplateID=".$TemplateID).'&amp;param=%NAME&fs='.+$_REQUEST['fs'].'">%NAME</a></td><td>%CAPTION</td><td>%TYPENAME</td><td align="center">'.nc_admin_checkbox_simple("kill[]", "%NAME").'</td></tr>', '</table>');
...
function nc_customsettings_show_once($ClassID = 0, $TemplateID = 0, $param = '', $Class_Template = 0) {
    ...
    foreach ($types as $v) {
        ...
        $f = new $classname ();
        $extend_params[$v] = $f->get_extend_parametrs();
        ...
        foreach ($subtypes[$v] as $k => $val) {
            ...
            $f = new $classname ();
            $extend_params[$v."_".$val] = $f->get_extend_parametrs();
...
function nc_customsettings_save_once() {
    ...
    $fl = new $classname();
    $ex_params = $fl->get_extend_parametrs();
    if (!empty($ex_params))
            foreach ($ex_params as $k => $v) {
            if ($type == 'select' && $subtype == 'static' && $k == 'values') {
                $param[$k] = array();
                if (!empty($_POST['select_static_key']))
                        foreach ($_POST['select_static_key'] as $key => $val) {
                        if (!$_POST['select_static_value'][$key] || !$val)
                                continue;
                        $param[$k][$val] = $_POST['select_static_value'][$key];
                    }
            }
            else {
...
function nc_admin_select_simple($disc, $name, $array, $selectedVar = 'nosel', $simple = '') {
    $ret = $disc."<select class='chzn-select' name='$name' $simple>";
