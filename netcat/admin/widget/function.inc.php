...
function nc_widget_list() {
    ...
    $select = "SELECT `Widget_Class_ID`, `Widget_ID`, `Name`, `Keyword` from `Widget` ORDER BY `Name`";
    if (($result = $db->get_results($select))) {
        echo "<script type='text/javascript'>function showhide(id){e=document.getElementById(id);if(e.style.display=='none'){e.style.display='block';}else{e.style.display='none';}}</script>".
        "<script type='text/javascript'>function filter() {widget_filter=document.getElementById(\"widget_filter\").value;tagList = document.getElementsByName('widget');for (var i = 0; i < tagList.length; i++){if ('widget_'+widget_filter != tagList.item(i).className && widget_filter != 0){tagList.item(i).style.display='none';} else if (widget_filter == 0){tagList.item(i).style.display='block';} else {tagList.item(i).style.display='block';}}}</script>".
        ...
        $categories = $db->get_results("SELECT `Category`, `Widget_Class_ID` FROM `Widget_Class` ORDER BY `Category`", ARRAY_A);

        $cat = (array) null;
        foreach ($categories as $c) {
            $new_cat[$c['Widget_Class_ID']] = $c['Category'];
            if (!in_array($c['Category'], $cat)) {
                $cat[] = $c['Category'];
            }
        }

        echo "".WIDGET_LIST_CATEGORY.":&nbsp;&nbsp;".
        ...
        foreach ($cat as $c) {
            echo "<option value='".md5($c)."'>".$c."</option>";
        }
        echo "</select><br /><br />";

        echo "
        <style>
            div.nc_idtab_buttons > div {
                padding-top: 7px;
                padding-left: 8px;
            }
        </style>";
        ...
        foreach ($result as $r) {
            echo "
                <div class='widget_".md5($new_cat[$r->Widget_Class_ID])."' name='widget' style='display:block;margin:0 0 17px 0;'>
                    <div style='padding-bottom: 5px;'>
                        {$r->Name}
                    </div>

                    <div>
                        <div class='nc_idtab' style='width: 94px; margin: 0px; padding: 0px; float: left; margin-right: 10px;'>
                            <div class='nc_idtab_buttons'>
                                <div>
                                    <a onClick='parent.nc_form(this.href); return false;' href='admin.php?phase=20&widget_id={$r->Widget_ID}&fs=".get_fs()."'>
                                        <div class='icons icon_copy' title ='" . NETCAT_MODERATION_COPY_OBJECT . "'></div>
                                    </a>
                                </div>

                                <div>
                                    <a onClick='parent.nc_form(this.href); return false;' href='admin.php?phase=30&widget_id={$r->Widget_ID}&fs=".get_fs()."'>
                                        <div class='icons icon_pencil' title ='" . NETCAT_MODERATION_CHANGE . "'></div>
                                    </a>
                                </div>

                                <div>
                                    <a onClick='parent.nc_action_message(this.href); return false;' href='admin.php?phase=61&widget_id={$r->Widget_ID}&fs=".get_fs()."'>
                        ...
                        <div style='height: 30px; padding-top: 17px;'>
                            <a href='#' onclick='showhide(\"".$r->Keyword."\");return false;' style='text-decoration:none;border-bottom:1px dashed;'>
                                ".WIDGET_LIST_INSERT_CODE."
                            </a>
                            &nbsp;&nbsp;
                            <a href='#' onclick='nc_widget_prev_obj.change(\"".$r->Keyword."\");return false;' style='text-decoration:none;border-bottom:1px dashed;'>
                                ".WIDGET_LIST_PREVIEW."
                            </a>
                        </div>
                    </div>
                    ...
                    <div class='nc_widget_prev' id='".$r->Keyword."' style='display:none;margin:15px 0 0 0;'>
                        <strong>
                            ".WIDGET_LIST_INSERT_CODE_CLASS.":
                        </strong>
                        &lt;?= \$nc_core->widget->show('".$r->Keyword."'); ?><br />
                        <strong>
                            ".WIDGET_LIST_INSERT_CODE_TEXT.":
                        </strong>
                        %NC_WIDGET_SHOW('".$r->Keyword."')%
                    </div>
                    <div class='nc_widget_prev' id='prev_load_".$r->Keyword."' style='display:none;'>2
                        ".WIDGET_LIST_LOAD."
                    </div>
                </div>
                <div class='nc_widget_prev' id='prev_".$r->Keyword."' style='display:none;'></div>
            </div>";
        }
    } else {
...
function nc_widgetclass_list($category) {
    ...
    if ($result) {
        ...
        "<th class='align-center'>".WIDGET_LIST_GO."</th>".
        "<th width='10%'>".WIDGET_LIST_FIELDS."</th>".
        "<td class='align-center'><div class='icons icon_delete' title='".WIDGET_LIST_DELETE."'></div></td>".
        "</tr>";
        foreach ($result as $key => $val) {
            if (md5($val['Category']) == $category || !isset($category)) {
                echo "<tr>".
                "<td >".$key."</td>".
                "<td ><a href='index.php?fs=".get_fs()."&phase=40&widgetclass_id=".$key."'>".$val['Name']."</a></td>".
                ...
                "<td><a href='".$ADMIN_PATH."field/index.php?fs=".get_fs()."&widgetclass_id=".$key."&fs=".get_fs()."'>".$val['Fields']." ".plural_form($val['Fields'], 'поле', 'поля', 'полей')."</a></td>".
                "<td align='center'>".nc_admin_checkbox_simple("Delete".$key, $key)."</td>".
                "</tr>";
            }
        }
...
function nc_widgetclass_edit_form($params='', $widgetclass_id='', $phase) {
    ...
    nc_print_admin_save_scritp('nc_widgetclass_form');
}
...
function nc_widgetclass_export($widget_class_id) {
    ...
    if ($result = $db->get_results($select, ARRAY_A)) {
        ...
        $ret = "<?xml version='1.0' encoding='utf-8'?>\n".
                ...
                "      <Template><![CDATA[".to_uni($result[0]['Template'])."]]></Template>\n".
                "      <Settings><![CDATA[".to_uni($result[0]['Settings'])."]]></Settings>\n".
                "      <AddForm><![CDATA[".to_uni($result[0]['AddForm'])."]]></AddForm>\n".
                "      <EditForm><![CDATA[".to_uni($result[0]['EditForm'])."]]></EditForm>\n".
...
function nc_widgetclass_import($import) {
    ...
    //$file = to_uni(file_get_contents($import['tmp_name']));
    ...
    if (isset($tar)) {
        ...
        $tar_name = $nc_core->TMP_FOLDER.$ID.'_temp.tgz';
        file_put_contents($tar_name, base64_decode(trim($tar)));
        nc_tgz_extract($tar_name, $nc_core->TMP_FOLDER);
        rename($nc_core->TMP_FOLDER.$ID, $nc_core->CLASS_TEMPLATE_FOLDER.$id);
        unlink($tar_name);
    }
    return $id;
}
...
function echo_remind_script() {
    echo "<!--  START   REMIND_SAVE --><script type='text/javascript'>remind_widgetclass();</script><!-- END REMIND_SAVE -->";
}
...
function echo_widgetclass_form($params, $phase='', $widget_group='') {
    ...
    if ( get_fs() ) {
        $widget_editor = new nc_widget_editor($nc_core->TEMPLATE_FOLDER, $nc_core->db);
...
function echo_widget_form($params, $alter = 0, $action = '') {
    ...
    if (!$alter) {
        $html .= "
            <legend>".WIDGETS_PARAMS."</legend>".
                "<form id='adminForm' method='post' $action><br />".
                "".WIDGET_ADD_NAME.":<br />".
                nc_admin_input_simple('Name', $params['Name'], 50)."<br /><br />".
                "".WIDGET_ADD_KEYWORD.":<br />".
                nc_admin_input_simple('Keyword', $params['Keyword'], 50)."<br /><br />";
    }
    ...
    if (!empty($widgets))
            $html .= nc_select_options($widgets, $params['Widget_Class_ID']);
