#!/usr/local/bin/php
<?php
/* $Id: crontab.php 5946 2012-01-17 10:44:36Z denis $ */
...
$HTTP_HOST = 'example.net'; #  <-------------= put domain here!!! Укажите здесь домен!!!
    ...
    if ($rs['Cron_Minutes'] or $rs['Cron_Hours'] or $rs['Cron_Days']) {
        if ($time <= time()) {
            ...
            if (substr($rs['Cron_Script_URL'], 0, 1) == "/") {
                $no_err = nc_task_fetch_url('http://'.$HTTP_HOST.$rs['Cron_Script_URL']);
                //passthru('wget -O - -q "http://'.$HTTP_HOST.$rs['Cron_Script_URL'].'"');
            } else {
                $no_err = nc_task_fetch_url($rs['Cron_Script_URL']);
