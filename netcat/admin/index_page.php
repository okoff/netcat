...
<html>
    <head>
        ...
        $modules = $db->get_results("SELECT Keyword, Module_Name FROM Module WHERE Checked=1 ORDER BY Keyword", ARRAY_A);
        if ( !empty($modules) ) {
    ...
    <body class='admin_form' style='margin: 0px;'>
        ...
        if ($perm->isAccessSiteMap() || $perm->isGuest()) {
            ...
            if ($favorites) {
                ...
                foreach ($favorites as $fkey=>$favorite) {
                    ...
                    if ($catalogue != $favorite['Catalogue_ID']) {
                        ...
                        echo "\t\t<div class='icons icon_site'></div>";
                    ...
                    echo "\t\t<div class='icons icon_folder".(!$favorite['SubChecked'] ? "_disabled" : "")."'></div>\n";
                    ...
                    echo "\t\t<div class='block_line_link'><img border='0' src=".$ADMIN_TEMPLATE."img/px.gif width='16' height='16' style='margin:0px 2px 0px 2px;'>".($subclass_id ? "<a href='http://".$EDIT_DOMAIN.$SUB_FOLDER.$HTTP_ROOT_PATH."?inside_admin=1&cc=".$subclass_id.(strlen(session_id()) > 0 ? "&".session_name()."=".session_id()."" : "")."'><div class='icons icon_pencil' title='".CONTROL_CONTENT_CATALOUGE_FUNCS_SHOWMENU_A_EDIT."' style='margin:0px 2px 0px 2px;'></div></a>" : "<img border='0' src=".$ADMIN_TEMPLATE."img/px.gif width='19' height='19' style='margin:0px 2px 0px 2px;'>")."<a href='".nc_subdivision_preview_link($favorite)."' target='_blank'><div class='icons icon_preview' title='".CONTROL_CONTENT_SUBDIVISION_FUNCS_TOVIEW."'></div></a></div>\n";
                    echo "\t</div>\n";
            ...
            echo "\t<div class='block_text'>\n";
            echo "\t\t<div class='block_line_link'><a href='subdivision/favorites.php?phase=1'><div class = 'icons icon_settings' title='".SECTION_INDEX_FAVORITE_SETTINGS."'></div>&nbsp;&nbsp;&nbsp;".SECTION_INDEX_FAVORITE_SETTINGS."</a></div>\n";
            echo "\t</div>\n";
            echo "</div>\n";
        }
        ...
        if ($perm->isSupervisor() || $perm->isGuest()) {
            ...
            if ($modules = $db->get_results("SELECT Module_ID,Module_Name,Keyword FROM Module")) {
                ...
                foreach ((array) $modules as $module) {
                    ...
                    echo "\t\t<div class='icons icon_module_".$module->Keyword."' title='".constant($module->Module_Name)."'></div>\n";
                    ...
                    echo "\t\t<div class='block_line_link'><a href='modules/index.php?phase=2&module_name=".$module->Keyword."'><div class = 'icons icon_settings' title='".TOOLS_MODULES_MOD_PREFS."' title='".TOOLS_MODULES_MOD_PREFS."'></div></a></div>\n";
                    echo "\t</div>\n";
            ...
            foreach ($real_modules as $key => $real_module) {
                if (!$have_modules[$key] && $key != 'eshop' && $key != 'forum') {
                    ...
                    $modules_must_have .= "\t\t<div class='icons icon_module_".$key."' title='".$real_module."' style='opacity: 0.4'></div>\n";
