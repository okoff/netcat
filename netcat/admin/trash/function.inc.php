...
function nc_trash_list($options = array()) {
    ...
    SELECT `t`.`Sub_Class_ID`, `t`.`Class_ID` , `t`.`Subdivision_ID`, t.`XML_Filename`, t.`XML_Filesize`,  UNIX_TIMESTAMP(t.`Created`) as `created`,
                    `sub`.`Subdivision_Name`, `cc`.`Sub_Class_Name`, c.`Class_Name`
    FROM (`Trash_Data` as `t`, `Class` as `c`)
    ...
    foreach ($sub_classes as $id => $v) {
        $sub_id = $sub_classes[$id]['sub_id'];
        $sub_name = $subdivisions[$sub_id]['name'];
        if (count($cc_sub[$sub_id]) > 1)
                $sub_name .= " (".$sub_classes[$id]['name'].")";
        echo "<div style='margin-bottom: 10px;'>
            ".nc_admin_img('i_folder.png', '')."
            ".$sub_id.". <a href='#' class='ajax_link' onclick='nc_trash_get_objects(".$id.", ".($options['date_b'] + 0).", ".($options['date_e'] + 0).");return false;'>
              ".$sub_name."</a>
          <div id='cc_".$id."' style='margin:10px 0 0 20px;'></div>
        </div>";
    }
        ...
        <form method='post' action='index.php'>
          <table id='trash_search'  class='admin_table'>
            ...
            nc_admin_input_simple('options[date_b_min]', $b_i, 2, '', "maxlength='2'")."
				...
				<td class= 'right'>
				<div class='calendar'>
                  <img  id='nc_calendar_popup_img_options[date_b_dd]' onclick='nc_calendar_popup(\"options[date_b_dd]\",\"options[date_b_mm]\", \"options[date_b_yyyy]\", \"0\");' src='".$nc_core->SUB_FOLDER.$nc_core->HTTP_ROOT_PATH."modules/calendar/images/calendar.jpg' />
                </div>
            ...
            nc_admin_input_simple('options[date_e_min]', $e_i, 2, '', "maxlength='2'")."
				...
				<td class= 'right'>
								<div class='calendar'>
                  <img  id='nc_calendar_popup_img_options[date_e_dd]' onclick='nc_calendar_popup(\"options[date_e_dd]\",\"options[date_e_mm]\", \"options[date_e_yyyy]\", \"0\");' src='".$nc_core->SUB_FOLDER.$nc_core->HTTP_ROOT_PATH."modules/calendar/images/calendar.jpg' />
                </div>
				</td>
