...
if ($_GET['action'] == 'get_path' && ($node_type == 'group' || $node_type == 'dataclass' || $node_type == 'field' || $node_type == 'widgetclass') && $node_id) {
    switch ($node_type) {
        case 'dataclass':
...
elseif ($node_type == 'dataclass.list') {
    ...
    foreach ((array) $class_groups as $class_group) {
        ...
        $ret_groups[] = array(
                ...
                "name" => $class_group["Class_Group"],
                "href" => "#classgroup.edit(".$class_group['Class_Group_md5'].")",
                "image" => 'icon_classgroup',
...
elseif ($node_type == 'systemclass.list') {
    ...
    foreach ((array) $system_fields as $system_field) {
        if ($system_field["TypeOfData_ID"] == 11 && !( $nc_core->modules->get_by_keyword('auth') && nc_auth_openid_possibility() ))
                continue;
        ...
        $ret_system_fields[] = array(
                ...
                "image" => $field_types[$system_field["TypeOfData_ID"]],
...
elseif (($node_type == 'template' && $node_id) || ($node_type == 'templates')) {
    ...
    $SQL = "SELECT `Template_ID`,
                   `Description`
                FROM `Template`
                    WHERE `Parent_Template_ID` = $node_id
                      AND `File_Mode` = $File_Mode
                        ORDER BY `Template_ID`";


    $templates = $db->get_results($SQL, ARRAY_A);

    foreach ((array) $templates as $template) {
        $hasChildren = $db->get_var("SELECT COUNT(*) FROM `Template`
      WHERE `Parent_Template_ID` = '".$template['Template_ID']."'");
        ...
        $template_buttons[] = nc_get_array_2json_button(
                ...
                "template$fs_suffix.add(".$template['Template_ID'].")",
                "icon_template_add");
        
        $template_buttons[] = nc_get_array_2json_button(
                CONTROL_TEMPLATE_DELETE,
                "template$fs_suffix.delete(".$template['Template_ID'].")",
                "icon_delete");
        
        $ret_templates[] = array(
                "nodeId" => "template-".$template['Template_ID'],
                "name" => $template["Template_ID"].". ".$template["Description"],
                "href" => "#template.edit(".$template['Template_ID'].")",
                "image" => 'icon_templates',
                "hasChildren" => $hasChildren,
                "dragEnabled" => false,
