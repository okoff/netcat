...
treeSelector = {
    ...
    changeMode: function(modeName, selectedNode) {
        ...
        if (treeSelector.currentMode == modeName) {
            if (selectedNode) {
                document.getElementById('treeIframe').contentWindow.tree.selectNode(selectedNode);
            }
            return;
        }

        document.getElementById('treeIframe').contentWindow.location =
...
function updateSysMsgIndicator(active) {
    var firstChild = document.getElementById('mainMenuMessagesIcon');//$nc('#mainMenuMessagesIcon');
    if ( firstChild )
        firstChild.setAttribute("class", 'icon_sysmsg_' + (active ? 'active' : 'inactive'));
}
