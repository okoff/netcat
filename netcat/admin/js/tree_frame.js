...
dynamicTree.prototype.addNode = function(nodeData) {
    ...
    if ( nodeData.sprite ) {
        ...
        oIcon.className = 'icon s_img s_img_' + nodeData.sprite;
    } else {
...
dynamicTree.prototype.loadChildren = function(nodeId, moreParams) {
    ...
    + this.instanceName+".loadChildren(\""
    + (nodeId != undefined ? nodeId : '')
    + "\", \""
    + (moreParams != undefined ? moreParams : '')
    + "\")')";
    var nodes = this.xhr.getJson(url, null, {
        '401': on401
    });

    // double load fix
    if ( this.areChildrenLoaded(nodeId) ) {
        return false;
    }
...
dynamicTree.prototype.moveHighlightSelected = function (nodeId) {
	...
	if (nodeId) {
        this.oSelectedNode = this.getNode(nodeId);
    }
    if (!this.oSelectedNode) {
    ...
    this.oHighlightSelected.style.top = (top-6)+'px';
...
function treeFieldAcceptDrop(e) {
    ...
    if (draggedParent.id == targetParent.id) {
        ...
        if (dragged.type=='field' && target.type=='field' && this.tagName=='IMG') return true;
    }
...
function treeSystemFieldAcceptDrop(e) {
    ...
    if (draggedParent.id == targetParent.id) {
        ...
        if (dragged.type=='systemfield' && target.type=='systemfield' && this.tagName=='IMG') return true;
    }
...
function treeGenericDropHandler(url) {
    var dragged = top.dragManager.draggedInstance,
    target  = top.dragManager.droppedInstance,
    position = (top.dragManager.dropTargetObject.tagName == 'DIV' ? 'below' : 'inside'),
    xhr = new httpRequest(),
    res = xhr.getJson(url,
    {
