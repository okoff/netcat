...
function SearchUserForm() {
    global $db, $ROOT_FOLDER, $INCLUDE_FOLDER, $admin_mode;
    global $systemTableID, $systemMessageID, $systemTableName, $ADMIN_PATH;
    global $UserID, $Checked, $grpID, $sort_by, $sort_order, $objcount, $nonConfirmed;
    ...
    $html .= "<legend ><span style='color: #1A87C2; border-bottom: 1px dashed;'>" . CONTROL_USER_FUNCS_USERSGET . "</span></legend>";
    $html .= "</fieldset>";
    ...
    $html .= "<span style='color: #1A87C2; border-bottom: 1px dashed;'>" . CONTROL_USER_FUNCS_USERSGET . "</span>:";
    $html .= "</legend>";
 ...
 */
function ListUserPages($totRows, $queryStr, $grpID, $Checked, $sort_by, $sort_order, $objcount) {
    global $db, $curPos;
...
function SearchUserResult() {
    ...
    $searchForm = SearchUserForm($order_by);
    //параметры поиска
    ...
    $select = "SELECT SQL_CALC_FOUND_ROWS a.`User_ID` AS id,  a.`Checked` AS checked, a.`" . $AUTHORIZE_BY . "` AS login,
             g.`PermissionGroup_ID` AS grp, GROUP_CONCAT( CONCAT(g.`PermissionGroup_ID`, '. ', g.`PermissionGroup_Name`) SEPARATOR '<br>') AS groups
             FROM `User` AS a,
             `User_Group` AS ug,
             `PermissionGroup` as g"
    ...
    if (!empty($Users)) {
        ...
        <form method='post' action='index.php' id='mainForm'>
                    ...
                    <td>
                        <table class='admin_table' width='100%'>
                            <tr>
                                ...
                                <? if ($edit_access) : ?>
								<th class='align-right' width='25%'><?= CONTROL_USER_ACTIONS ?></th>
                                <? endif;
                                if ($right_access): ?>
								<th class='align-center'><?= CONTROL_USER_RIGHTS ?></th>
                                <? endif;
                                if ($del_access): ?>
            ...
            <td >\n";
                                ...
                                print (($AUTHORIZE_BY != "User_ID") && !empty($User['login'])) ? $User['login'] : CONTROL_USER_EDIT;
                                print "</a></td>";
                                
                                print "<td>" . $User['groups'] . "</td>";
                                ...
                                if ($right_access) {
                                    ...
                                    if ($this_user_edit)
                                        print "<a href=\"index.php?phase=8&UserID=" . $User['id'] . "\"><div class='icons icon_settings' title='" . CONTROL_USER_FUNCS_EDITACCESSRIGHT . "'></div></a>";
                                    print "</td>";
                                }
            ...
            <table border=0 cellpadding=0 cellspacing=0 width=100%>
                <tr>
                    ...
                    <tr>
                        ...
                        <td align=center><a href=\"group.php?phase=8&PermissionGroupID=" . $Array[0] . "\"><div class='icons icon_settings' title='" . CONTROL_USER_CHANGERIGHTS . "'></div></a></td>";
...
###############################################################################
    ...
    function UserForm($UserID, $action_file, $phase, $type) {
        ...
        global $nc_core, $db, $ROOT_FOLDER, $admin_mode, $DOMAIN_NAME;
        ...
        echo "<br /><form name='adminForm' id='adminForm' " . ($is_there_any_files ? "enctype='multipart/form-data'" : "") . " method='post' action='" . $action_file . "'>";
        ...
        if ($db->num_rows < 8) { // Если групп мало, то выводим через checkbox
            ...
            foreach ($Result as $Group) {
                ...
                echo nc_admin_checkbox_simple("PermissionGroupID[" . $id . "]", $id, $id . ":" . $name, in_array($id, (array) $User['PermissionGroup_ID']), "grp_" . $id) . "<br>";
            }
            echo "<br>";
        } else {
            ...
            foreach ($Result as $Group) {
                $id = $Group['PermissionGroup_ID'];
                $name = $Group['PermissionGroup_Name'];
                echo "<option" . ( (in_array($id, (array) $User['PermissionGroup_ID'])) ? " selected" : "") . " value='" . $id . "'>" . $id . ":" . $name . "</option>";
            }
            echo "</select><br><br>";
        }
...
###############################################################################
    ...
    <?php echo $nc_core->token->get_input(); ?>
                        ...
                        <?
                        if ($UserID) {
                            $UI_CONFIG->headerText = CONTROL_USER_RIGHT_ADDPERM . " " . $user_login;
                        } else {
                    ...
                    class ui_config_user extends ui_config {
                        ...
                        function user_page($user_id, $user_login, $active_tab, $hash = "") {
                            ...
                            $this->headerText = CONTROL_USER . " $user_login";
