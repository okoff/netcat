<?php
/* $Id: function.inc.php 8481 2012-11-29 08:22:23Z lemonade $ */
        ...
        if (!empty($fl)) {
            ...
            foreach ($fl as $field) {
                if ($real_value[$field['usage']] && $field['usage'] && $nc_core->input->fetch_get_post($field['name']) && $nc_core->input->fetch_get_post($field['name']) != $real_value[$field['usage']]) {
                    nc_print_status(sprintf(CONTROL_CONTENT_SUBDIVISION_SEO_VALUE_NOT_SETTINGS, $field['usage'], $field['usage']), 'info');
                }
            }
        }
...
function ShowFullSubdivisionList() {
    ...
    echo "<tr>
      <td id='siteTitle' class='name ".($Array[4] ? "active" : "unactive")."'>
          <span>".$Array[0].". </span>
          <a href='".$ADMIN_PATH."catalogue/index.php?phase=2&type=2&CatalogueID={$Array[0]}'>".$Array[1]."</a>
      </td>

      <td class='button'>".($is_supervisor ? "<a href='index.php?phase=2&CatalogueID=".$Array[0]."&ParentSubID=0'><div class='icons icon_folder_add' title='".CONTROL_CONTENT_SUBDIVISION_FUNCS_MAINDATA_A_ADDSUBSECTION."'></div></a>" : "<img src='".$ADMIN_TEMPLATE."img/px.gif' alt='' />")."</td>
      <td class='button_div'><img src='".$ADMIN_TEMPLATE."img/px.gif' alt='' /></td>
      <td class='button'>".($is_supervisor ? "<a href='".$ADMIN_PATH."catalogue/index.php?phase=2&CatalogueID=".$Array[0]."&type=2'><div class='icons icon_settings' title='".CONTROL_CONTENT_CATALOUGE_FUNCS_SHOWCATALOGUELIST_TOOPTIONS."'></div></a>" : "<img src='".$ADMIN_TEMPLATE."img/px.gif' alt='' />")."</td>
      <td class='button'>";
    ...
    echo "</td>
      <td class='button'><a href='http://".($Array[2] ? $Array[2] : $DOMAIN_NAME).($SUB_FOLDER ? $SUB_FOLDER."/" : NULL).(strlen(session_id()) > 0 ? "?".session_name()."=".session_id()."" : "")."' target=_blank><div class='icons icon_preview' title='".CONTROL_CONTENT_CATALOUGE_FUNCS_SHOWMENU_A_VIEW."'></div></a></td>
      <td class='button_div'><img src='".$ADMIN_TEMPLATE."img/px.gif' alt='' /></td>
      <td class='button'>".($is_supervisor ? "<a href='".$ADMIN_PATH."catalogue/?".$nc_core->token->get_url()."&phase=4&Delete".$Array[0]."=".$Array[0]."'><div class='icons icon_delete' title='".CONTROL_CONTENT_SUBDIVISION_FUNCS_MAINDATA_A_REMSITE."'></div></a>" : "<img src='".$ADMIN_TEMPLATE."img/px.gif' alt='' />")."</td>
      </tr>";
...
function write_sub($ParentSubID, $CatalogueID, $count = 1) {
    ...
    foreach ($Result as $Array) {
        $result .= "<tr>
      <td class='name ".($Array[3] ? "active" : "unactive")."' style='padding-left: ".($count + 15)."px;'>
      ...
      <td class='button'><a href='".$ADMIN_PATH."subdivision/index.php?phase=2&ParentSubID=".$Array[0]."'><div class='icons icon_folder_add' title='".CONTROL_CONTENT_SUBDIVISION_FUNCS_MAINDATA_A_ADDSUBSECTION."'></div></a></td>
      <td class='button_div'><img src='".$ADMIN_TEMPLATE."img/px.gif' alt='' /></td>
      <td class='button'><a href='".$ADMIN_PATH."subdivision/index.php?view=edit&phase=5&SubdivisionID=".$Array[0]."'><div class='icons icon_settings' title='".CONTROL_CONTENT_SUBDIVISION_FUNCS_TOOPTIONS."'></div></a></td>";
        $show_url_arr = array("Hidden_URL" => $Array[4], "Domain" => $Array[5], "ExternalURL" => $Array[7]);

        $result .= "<td class='button'>";
        if (GetSubClassCount($Array[0])) {
            $cc_id = $db->get_var("SELECT Sub_Class_ID FROM Sub_Class WHERE Subdivision_ID = '".$Array[0]."'");
            $result .= "<a href='http://".$EDIT_DOMAIN.$SUB_FOLDER.$HTTP_ROOT_PATH."?inside_admin=1&cc=".$cc_id.(strlen(session_id()) > 0 ? "&".session_name()."=".session_id()."" : "")."'><div class='icons icon_pencil' title='".CONTROL_CONTENT_CATALOUGE_FUNCS_SHOWMENU_A_EDIT."'></div></a>";
        } else {
            $result.= "<img src='".$ADMIN_TEMPLATE."img/px.gif' alt='' />";
        }
        $result.= "</td>";
        $result .= "<td class='button'><a href='".nc_subdivision_preview_link($show_url_arr)."' target='_blank'><div class='icons icon_preview' title='".CONTROL_CONTENT_CATALOUGE_FUNCS_SHOWMENU_A_VIEW."'></div></a></td>";
        $result.= "<td class='button_div'><img src='".$ADMIN_TEMPLATE."img/px.gif' alt='' /></td>";
        $result.= "<td class='button'><input name='Delete".$Array[0]."' value='".$Array[0]."' type='checkbox'";

        if (in_array($Array[0], $subs_serv)) {
...
function nc_subdivision_show_edit_form($sub_id, $view) {
    ...
    echo "<form id='adminForm' name='adminForm' action='{$nc_core->ADMIN_PATH}subdivision/index.php' method='post' enctype='multipart/form-data' >";
...
function nc_subdivision_show_access($SubEnv, $have_default = true) {
    ...
    $html = "
        <style>
            div.nc_table_access > div {
                border-bottom: 1px #cccccc solid;
                display: table;
            }

            div.nc_table_access > div > div {
                display: inline-block;
                padding-top: 9px;
                padding-bottom: 11px;
            }

            div.nc_table_access div.col_1 {
                width: 96px;
            }

            div.nc_table_access div.col_2 {
                text-align: center;
                width: 132px;
            }

            div.nc_table_access div.col_3 {
                text-align: center;
                width: 79px;
            }

            div.nc_table_access div.col_4 {
                text-align: center;
                width: 180px;
            }

            div.nc_table_access div.col_5 {
                text-align: center;
                width: 158px;
            }

            div.nc_table_access > div.row_1 {
                padding-top: 3px;
                padding-bottom: 2px;
            }

            div.nc_table_access div.col_checked {
                background-color: #eeeeee;
            }
        </style>

        <div class='nc_table_access'>
            <div class='row_1'>
                <div class='col_1'>
                </div>";
    ...
    $html .= "  <div class='col_3'>
                    ".CLASSIFICATOR_USERGROUP_ALL."
                </div>

                <div class='col_4'>
                    ".CLASSIFICATOR_USERGROUP_REGISTERED."
                </div>

                <div class='col_5'>
                    ".CLASSIFICATOR_USERGROUP_AUTHORIZED."
                </div>
            </div>";
    ...
    foreach ($actions as $v) {
        $html .= "<div>
                      <div class='col_1'>
                          ".constant("CONTROL_CONTENT_SUBDIVISION_FUNCS_ACCESS_".strtoupper($v))."
                      </div>";

        if($have_default) {
            $html .= "<div class='col_2'>
                            <input name='{$v}_Access_ID' type='radio' ".(!$SubEnv['_db_'.$v."_Access_ID"] ? " checked='checked'" : "")." value='0' />
                      </div>";
        }

        for ($i = 0; $i < count($AccessType); $i++) {
            $html .= "<div class='col_".($i + 3).($AccessType[$i][0] == $SubEnv[$v."_Access_ID"] ? ' col_checked' : '')."'>
                          <input name='{$v}_Access_ID' ".($AccessType[$i][0] == $SubEnv['_db_'.$v."_Access_ID"] ? " checked='checked'" : "")." type='radio' value='{$AccessType[$i][0]}' />
                      </div>";
        }
    ...
    $html .= "</div><br /><div>".CONTROL_CONTENT_SUBDIVISION_FUNCS_ACCESS_PUBLISH."</div>";
    $html .= nc_get_modal_radio('Moderation_ID', $radio_array, +$SubEnv["_db_Moderation_ID"]);
...
function nc_subdivision_form_edit($sub_env) {
    ...
    $cc_in_sub = $nc_core->db->get_row("SELECT sc.`Sub_Class_ID`, sc.`CustomSettings`, sc.`Sub_Class_Name`, c.`CustomSettingsTemplate`, c.`Class_ID`
                                FROM `Sub_Class` AS `sc`, `Class` AS `c`
                                WHERE c.`Class_ID` = sc.`Class_ID`
                                AND sc.`Subdivision_ID` = '".intval($sub_id)."'
                                ORDER BY sc.`Checked` DESC, sc.`Priority` LIMIT 1", ARRAY_A);
    if ($cc_in_sub['CustomSettingsTemplate']) {
        $field_cs = new nc_admin_fieldset(CONTROL_CONTENT_SUBDIVISION_SETTINGS_TEMPLATE." \"".$cc_in_sub['Sub_Class_Name']."\"");
        $field_cs->add("<input type='hidden' name='custom_class_id' value='".$cc_in_sub['Class_ID']."' />");
        $field_cs->add("<input type='hidden' name='custom_subclass_id' value='".$cc_in_sub['Sub_Class_ID']."' />");

        require_once($nc_core->ADMIN_FOLDER."array_to_form.inc.php");
        $values = ($CustomSettings ? $CustomSettings : $cc_in_sub['CustomSettings']);
        $a2f = new nc_a2f($cc_in_sub['CustomSettingsTemplate'], 'CustomSettings');
        $a2f->set_value($values);
        // this is only for inside_admin mode
        $vs_template_header = "<br/><table cellpadding='0' cellspacing='0' class='ncf_table border-bottom' width='98%'>
                <colgroup>
                    <col width='15%'>
                    <col width='70%'>
                    <col width='15%'>
                </colgroup>".
                "<tr align='center'><td class='ncf_td_header_caption'>%CAPTION</td><td class='ncf_td_header_value'>%VALUE</td><td class='ncf_td_header_default'>%DEFAULT</td></tr>";
        $vs_template_object = "<tr><td class='ncf_td_caption'>%CAPTION</td><td class='ncf_td_value'>%VALUE</td><td class='ncf_td_default'>%DEFAULT<br /></td></tr>";
        $vs_template_footer = "</table><br />";
        $vs_template_divider = "<tr><td colspan='3' class='ncf_td_divider'>%CAPTION</td></tr>";

        $field_cs->add($a2f->render($vs_template_header, $vs_template_object, $vs_template_footer, $vs_template_divider));

        $html .= $field_cs->result();
    }
...
function nc_subdivision_form_design($sub_env, $CatalogueID = null) {
    global $perm, $loc;
    $nc_core = nc_Core::get_object();
    $db = $nc_core->db;
    $html = '';
        ...
        if ($settings) {
            ...
            if (!$a2f->validate($_POST['CustomSettings'])) {
                ...
                if ($CustomSettings <> $cur_settings) {
                  ...
                  WHERE `Sub_Class_ID` = '".$custom_subclass_id."'");
                    $nc_core->event->execute("updateSubClass", $cat_id, $sub_id, $custom_subclass_id, 0);
                }
            }
        }
    }
...
function nc_subdivision_form_design_save() {
    ...
    $fields = array('Template_ID' => $is_parent_template ? '' : $Template_ID, 'TemplateSettings' => $TemplateSettings, 'UseEditDesignTemplate' => $UseEditDesignTemplate);
    $nc_core->subdivision->update($sub_id, $fields);
...
function nc_subdivision_show_component($catalogue_id = 0) {
    ...
    $classes = $nc_core->db->get_results("SELECT `Class_ID` as value, CONCAT(`Class_ID`, '. ', `Class_Name`) as description, `Class_Group` as optgroup
    FROM `Class`
    ...
    if (!empty($classes)) {
        ...
        $html .= nc_select_options($classes, $Class_ID);
        $html .= "</select>";
    ...
    if (!empty($classes)) {
        ...
        $html .= "<script type='text/javascript'>if (".($ClassID + 0).") loadClassCustomSettings(".($Class_Template_ID ? $Class_Template_ID : $Class_ID).")</script>";
    }
      ...
      (`Subdivision_ID`, `Catalogue_ID`, `Class_ID`, `Sub_Class_Name`, `Checked`, `EnglishName`, `Created`, `CustomSettings`, `Class_Template_ID`)
       ...
       ('".$SubdivisionID."', '".$CatalogueID."', '".$Class_ID."', '".$Subdivision_Name."', 1, '".$EnglishName."',  '".date("Y-m-d H:i:s")."',  '".addcslashes($CustomSettings, "'")."', '".$Class_Template_ID."')");
        ...
        if (($SubClassID = $db->insert_id)) {
            $nc_core->event->execute("addSubClass", $CatalogueID, $SubdivisionID, $SubClassID, $Class_ID);
        }
    }
...
function nc_get_modal_radio($name, array $items, $checked_value) {
    ...
    foreach ($items as $item) {
        ...
        $item['attr']['style'] = $item['attr']['style'] ? $item['attr']['style'] : 'margin: 0px; padding: 0px;';
        ...
        $html .= "
            <div>
                <div class='nc_radio_modal_input'>
