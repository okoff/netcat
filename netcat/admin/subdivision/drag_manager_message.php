...
if ($dragged_type == 'message' && $target_type == 'subclass') {
    ...
    if (!$subclass || $subclass['Class_ID'] != $dragged_class) {
        die("0 /* Can't move message between distinct classes or subclass [$target_id] doesn't exist */");
    }

    $res = nc_move_message($dragged_class, $dragged_id, $target_id);
    if (!$res) {
